...
import com.company.wallet.backend.service.directory.AccountNotFoundException
import com.company.wallet.domain.error.DuplicateRequestException
import com.company.wallet.domain.error.UnsupportedCapabilityException
 ...
 *
 * 생성자 주입은 포트 타입만 받는다 — 어떤 custody 어댑터(현재 Fireblocks, 테스트는 fake)가 꽂히는지
...
class WalletGatewayService(
     ...
     * [DepositAddressIssuanceCapability] 를 구현하지 않으면 명시적 거절로 끝낸다.
     */
    suspend fun issueDepositAddress(
        ...
        val issuer =
            accounts as? DepositAddressIssuanceCapability
                ?: throw UnsupportedCapabilityException("issueDepositAddress")
        return withIdempotency(
