...
class SelfBuildAdapter(
    ...
    /** 임의 주소 이체 이력 — 도메인 동사, 인덱서 projection 경유 (가이드 13.3). */
    override suspend fun transfersOf(
        address: Address,
        range: BlockRange,
    ): List<Transfer> = projections.transfersOf(address, range)

    /** as-of-block 잔액 — 도메인 동사, 인덱서 projection 경유 (가이드 2.6 · 13.3). */
    ...
    /** 탈출구 — 동사로 못 담는 커스텀·집계 조회, 인덱서 projection 경유 (가이드 15.2 query, 13.4). */
