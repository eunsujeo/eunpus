...
| 경로 | 내용 |
|---|---|
| `confluence/` | Confluence 붙여넣기용 정리 문서 7편 — 가이드 전체가 아니라 회사 공유에 필요한 핵심만 압축. Confluence wiki markup 형식 (`{plantuml}` · `{info}` 매크로 사용). `06-design-rationale.txt` 는 컴포넌트별 설계 사유("왜") + 기각한 대안 모음 |
...
3. 문서 순서: 00 → 06. 각 문서 끝의 "더 보기" 가 원본 가이드 페이지를 가리킨다.
