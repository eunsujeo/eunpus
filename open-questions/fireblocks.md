...
- **과금 후속 확답 (Stage 176, 2026-09-14 사용자 전달)**: 가격 범위는 월 구독료 티어이고 실제 가스비는 별도 상환한다. 티어 건수는 최초 EIP-7702 업그레이드를 포함해 Fireblocks가 가스비를 지불한 모든 거래다. 첫 업그레이드와 전송이 별도 거래인지는 이번 답변에 명시되지 않았다. (source: [담당자 답변](../blockchain-manager/sources/fireblocks-support/2026-09-14__gasless-pricing-tier-confirmation.md))
- **수수료 필드 보강 (Stage 177, 타인의 담당자 Q&A)**: `feeInfo`에 Relay 대납 실비·Revert 실비가 남으며, Webhook `relayType`은 self-relay도 `THIRD_PARTY`로 반환된다는 설명을 확보했다. `feeUSD`는 표시 전용으로 월 청구·정산에 사용되지 않는다. 정산용 USD 환율 기준은 여전히 미확정이며 우리 Workspace의 payload 실측도 남아 있다. (source: [2026-09-14 사용자 전달 답변](../blockchain-manager/sources/fireblocks-support/2026-09-14__gasless-feeinfo-third-party-qna.md))
- **Status**: ANSWERED (잔여 CSM/PoC: 정확한 계약 금액·가스비 USD 환산 기준 · MPC↔7702 위임 내부 동작 · expiresAfterSeconds↔transactionTimeout 명칭 관계)
