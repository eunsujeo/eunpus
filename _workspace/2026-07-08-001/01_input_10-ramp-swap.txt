...
세 거래 모두 **DB 는 즉시, 블록체인 전송은 15분 배치**라는 2단계로 돕습니다. 매니저(별도 서비스)는 **2단계의 블록체인 전송·감지**에서만 등장합니다. 1단계(DB 반영)엔 매니저 API 호출이 없습니다.
...
수수료는 두 층으로 갈립니다. **매니저 층**의 estimateFee API(7페이지)는 **변동 네트워크 실비**를 추정할 뿐, 실비를 미리 고정하지 않습니다. 반면 **고객에게 보여주는 수수료**는 그 위층(회계·비즈니스)이 **선계산해 고정한 값**입니다 — 자산별 고정 출금 수수료·% 매매 요율 같은 고정 스케줄이죠. 위층이 고정값을 약속할 수 있는 건, 변동 실비와의 **차액을 수수료수익/손익으로 흡수**하기 때문입니다. 정산·회계 처리는 정산 워크스루에서 다룹니다.
