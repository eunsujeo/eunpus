...
- **Canton Coin = burn-mint equilibrium** (★ Stage 54, 수치 60, **정정 Stage 216**) — 수수료(USD 표시·CC 지불)는 **소각=유통에서 제거**, **validator/app provider/SV** 가 **mint 보상**. **라운드 10분(연 ~52,560), 라운드마다 dev fund 5%(CIP-0082) 선차감 후 validator/app/SV 배분**, unclaimed cascade. **CC-USD = 라운드별 SV 게시율 median**. → burn 모델 확정. **(★ Stage 216 정정) liveness 보상은 폐지 — CIP-0096 으로 `validatorFaucetCap` 이 2026-04-30 부터 $0**(종전 기재 "$2.85/라운드" 는 그 이전 값). validator 수입은 **소각량 비례 `ValidatorRewardCoupon` 활동 보상뿐**. app 보상은 **featured 앱만**이고 산정은 **CIP-0104 traffic 기반**(미적용 망은 legacy `FeaturedAppActivityMarker`). CC 보유·이력은 **scan 서비스로 공개**(수수료 토큰에는 앱 거래 프라이버시가 적용되지 않음). (source: docs-canton-network-renewed tokenomics-of-gs, cf-docs-canton-2026-09-17)
...
- cf-docs-canton-2026-09-17 (2026-09-17) — Canton 통합 문서 <https://docs.canton.network/> 및 CIP 저장소 <https://github.com/canton-foundation/cips> 의 커밋 고정 스냅샷 8건(contract-keys · canton-coin-tokenomics · tokenomics-of-gs · synchronizer-traffic · token-standard · allocation v1/v2 OpenAPI · CIP-0112). `blockchain-manager/sources/canton-network/manifest.yml` 에 SHA-256 기록 (1차)
