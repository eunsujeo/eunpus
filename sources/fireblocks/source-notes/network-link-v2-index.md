...
## 우리 용도 — 제3자를 연결해 쓰는 쪽 (2026-08-11 확정)

이 스펙은 **제공자가 되는 쪽**의 규격이다. 우리는 제공자가 아니라 **이 규격을 구현한 제3자를 연결해 쓰는 쪽**이다. 따라서 **우리가 구현할 것은 없다** — 상대(거래소 등)가 이 API 를 구현하고, 우리는 Fireblocks 를 통해 그 상대에 연결한다.

그러면 이 스펙에서 우리에게 쓸모 있는 부분은 "무엇을 구현하나" 가 아니라 **상대가 무엇을 제공할 수 있는지의 목록**이다.

- `GET /capabilities` — 상대가 어떤 컴포넌트를 지원하는지가 여기서 갈린다. 컴포넌트별로 `"*"` 또는 특정 sub-account 목록으로 답하므로, **상대에 따라 쓸 수 있는 기능이 다르다.**
- `transfersBlockchain` 지원 여부 — 공개 블록체인으로 출금·입금 주소 발급이 되는지
- `liquidity` 지원 여부 — 자산 간 견적·실행(quote → execute)이 되는지
- `ramps` 지원 여부 — 램프가 되는지
- `collateral` — Off-Exchange 담보. 우리가 쓸 그림인지는 별개 판단

**다음에 볼 것은 이 스펙이 아니다.** 우리 쪽에서 필요한 것은 "워크스페이스가 Provider Network 의 상대에 어떻게 연결되고, 그 연결이 우리 API·정책·거래 기록에 어떻게 보이는가" 다. 그건 Fireblocks 쪽 문서(network connection·third-party account 계열)이고 이 저장소에 아직 없다.
