...
## (17) 잔여 batch + API/stdlib 표본 — Stage 62 (sweep 종료)

source: appdev/deep-dives/external-signing-transactions · overview/reference/{ledger-causality,decentralization} · appdev/reference/admin-api-reference · daml-standard-library/da-list (표본)

- **external-signing-transactions**: `PrepareSubmission`→DamlTransaction+metadata(synchronizerId·tx UUID·timing)+pre-hash(재계산 권장) → 서명(ECDSA-SHA256 또는 Ed25519, SIGNATURE_FORMAT_CONCAT, signed_by fingerprint) → `ExecuteSubmission(AndWait)`+PartySignatures+**submission_id(재시도 시 재서명 불필요)**. disclosed_contracts in PrepareSubmissionRequest. min_ledger_time/max_record_time, package_id_selection_preference, prepare 응답에 traffic 추정. 현재 single-party submission 만 지원.
- **ledger-causality (★ 수탁 리스크)**: Canton 은 **partial ordering**(total 아님) — party 마다 두 tx 를 다른 순서로 관찰 가능, causality graph(DAG)로 정합. **ledger time**(submitter, getTime()) vs **record time**(storage), bounded skew `rt-skew_min ≤ lt ≤ rt+skew_max`, **둘 다 단조증가 아님(시간 "약간 fuzzy")**. divulged 정보엔 **순서 보장 없음** → custodian 이 정식 원장 반영 전 divulged settlement 정보로 행동할 위험, multi-host party 는 동일 tx 를 다른 순서로 받을 수 있음(local projection 은 결국 수렴).
- **decentralization**: 기존 확정 재확인(decentralized namespace threshold·mediator group threshold·multi-sequencer BFT·"no trustee unilaterally"). SV 수 등 구체 수치 없음.
- **admin-api-reference**: 일부 서술 있음 — 보안 경고 **"Never expose the Admin API to the public internet"**, 서비스군(Health/Topology/Synchronizer/Package/Key Mgmt/Pruning/Repair). 나머지는 기계적.
- **API/stdlib 자동생성 bulk 판정**: da-list 표본 = 순수 함수 시그니처·타입(서술 없음). → **API Reference(~250: Protobuf/gRPC/Java/JSON) + Daml stdlib(~120) + 언어 레퍼런스는 promote 가능한 서술 사실 없음**(개별 fetch 불필요). 일부 hand-written reference(admin-api 등)의 보안 경고만 산발적. **sweep 종료**: 수탁/아키텍처 관련 content 페이지는 batch1~7 로 전수 커버, 나머지는 기계적 ref 로 범위 밖 확정.
