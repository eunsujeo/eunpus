...
(async () => {
  ...
  for (let i = 0; i < jobs.length; i += 1) {
    const job = jobs[i];
    // md.js fixMermaidListLabels 와 같은 변환 — "1. " 라벨의 list 오인 방지 (ZWSP 삽입)
    const fixed = job.source.replace(/(\\d+)\\.(?=\\s)/g, '$1.\\u200B');
    const rendered = await mermaid.render('export-mermaid-' + (i + 1), fixed);
