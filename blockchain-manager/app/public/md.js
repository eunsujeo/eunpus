...
window.MD = (() => {
  ...
  // Mermaid 11 은 노드·엣지 라벨을 markdown 으로 해석해 "1. " 로 시작하는 라벨을
  // ordered list 로 오인한다("Unsupported markdown: list"). 렌더 직전에 숫자 뒤 마침표와
  // 공백 사이에 zero-width space(U+200B)를 끼워 list 판정만 깨고 표시는 "1." 그대로 둔다.
  // #46; escape 는 <br/> 포함 라벨에서 markdown 파싱 전에 디코드돼 소용없음 — headless 실측 결과.
  // 문서 원문은 손대지 않는다. (export.js 브라우저 사전 렌더와 app/scripts/export-board.mjs 의
  // chrome 사전 렌더도 같은 변환을 쓴다)
  function fixMermaidListLabels(src) {
    return src.replace(/(\d+)\.(?=\s)/g, '$1.\u200B');
  }
  ...
  async function runMermaid(selector) {
    ...
    for (const el of pending) {
      const source = fixMermaidListLabels(el.textContent.trim());
      try {
  ...
  function stepAnim(el, cfg) {
    ...
    el.tabIndex = 0; // 화살표 키 탐색 — 애니메이션을 클릭·포커스한 뒤 ←/→
    el.addEventListener('keydown', (e) => {
      if (e.key === 'ArrowLeft') { stop(); go(step - 1); e.preventDefault(); }
      else if (e.key === 'ArrowRight') { stop(); go(step + 1); e.preventDefault(); }
    });
  ...
  // x402-payment — 자기 자신을 그려 가는 시퀀스 다이어그램: 레인 4개 + 화살표 + 이동 패킷 (에이전트 결제 1장)
  function buildX402Payment(el) {
    const LANE = { A: 12.5, S: 37.5, F: 62.5, C: 87.5 }; // 레인 중심 (%)
    // 기본 화면은 현재 단계 데이터 한 줄(fldline)만, 전체 필드는 "전체 JSON" 버튼의 모달(dialog)로 분리.
    // 값은 transport 스펙 예시 헤더를 디코드한 실값 — 한 줄 요약에선 축약, 모달에 원문.
    const FV = {
      ...
      sigFull: '0x2d6a7588d6acca505cbf0d9a4a227e0c52c6c34008c8e8986a1283259764173608a2ce6496642e377d6da8dbbf5836e9bd15092f9ecab05ded3d6293af148b571c',
    };
    const ab = (v) => v.slice(0, 6) + '…' + v.slice(-4);
    const REQ_OBJ = {
      x402Version: 2,
      error: 'PAYMENT-SIGNATURE header is required',
      resource: { url: FV.url, description: 'Access to premium market data', mimeType: 'application/json' },
      accepts: [{ scheme: 'exact', network: 'eip155:84532', amount: '10000', asset: FV.asset, payTo: FV.payTo, maxTimeoutSeconds: 60, extra: { name: 'USDC', version: '2' } }],
      extensions: {},
    };
    const PAY_OBJ = {
      x402Version: 2,
      resource: REQ_OBJ.resource,
      accepted: REQ_OBJ.accepts[0],
      payload: {
        signature: FV.sigFull,
        authorization: { from: FV.from, to: FV.payTo, value: '10000', validAfter: '1740672089', validBefore: '1740672154', nonce: FV.nonce },
      },
      extensions: {},
    };
    // 한 화면 유지: 카드 패널 대신 현재 단계의 핵심 데이터 한 줄(fldline) + 전체 필드는 레이아웃 밖 모달(dialog)
    const FLD = [
      '첫 요청 — 아직 결제 데이터 없음',
      ...
      '200 = 정산 성공 확인 → nonce "' + ab(FV.nonce) + '" <span class="cv">소진</span>. 응답 유실(성공 불명) 시 새 서명 전에 tx 해시·기록으로 기존 정산부터 확인 — 새 nonce 서명은 새 결제',
    ];
    const x402Dialog = () =>
      `<dialog class="x402-dialog"><div class="x402-dialog-head">스펙 예시를 바탕으로 재구성한 전체 필드 — 두 HTTP 헤더의 base64 디코드 형태` +
      `<button type="button" data-x402-close aria-label="닫기">✕ 닫기</button></div>` +
      `<div class="x402-json-grid">` +
      `<div><div class="x402-json-t">PaymentRequired — PAYMENT-REQUIRED 응답 헤더</div><pre>${JSON.stringify(REQ_OBJ, null, 2)}</pre></div>` +
      `<div><div class="x402-json-t">PaymentPayload — PAYMENT-SIGNATURE 요청 헤더</div><pre>${JSON.stringify(PAY_OBJ, null, 2)}</pre></div>` +
      `</div></dialog>`;
    const wire = (on, from, to, label, fld, seq2) => {  // seq2: 같은 단계의 두 번째 메시지 — 순차로 그린다
      if (!to) { // 내부 동작 — 레인 위 점선 칩
        return `<div class="banim-wire self" data-on="${on}" data-mstep="${on}" data-from="${from}" style="left:${LANE[from]}%">` +
          `<span class="wl"${fld ? ` data-f="${fld}"` : ''}>${label}</span></div>`;
      }
      const a = LANE[from], b = LANE[to];
      const dir = b > a ? 'ltr' : 'rtl';
      return `<div class="banim-wire ${dir}${seq2 ? ' seq2' : ''}" data-on="${on}" data-mstep="${on}" data-from="${from}" data-to="${to}" ` +
        `style="left:${Math.min(a, b)}%;width:${Math.abs(b - a)}%">` +
        `<span class="wl"${fld ? ` data-f="${fld}"` : ''}>${label}</span><i class="wline"></i><i class="pkt"></i></div>`;
    };
    ...
    stepAnim(el, {
      ...
      scene:
        '<div class="banim-actors seq">' +
        '<div class="banim-actor" data-a="A"><b>에이전트</b>지갑 · 서명</div>' +
        '<div class="banim-actor" data-a="S"><b>서버</b>merchant · 자원</div>' +
        '<div class="banim-actor" data-a="F"><b>Facilitator</b>검증 · 정산 대행</div>' +
        '<div class="banim-actor" data-a="C"><b>체인</b>EVM — 예시</div>' +
        '</div>' +
        '<div class="banim-seq">' +
        Object.values(LANE).map((x) => `<i class="banim-ll" style="left:${x}%"></i>`).join('') +
        wire(0, 'A', 'S', 'GET /premium-data') +
        wire(1, 'S', 'A', '402 + PaymentRequirements — 10,000 · payTo 0x2096…') +
        wire(2, 'A', '', '', 'sig') +
        wire(3, 'A', 'S', '같은 GET + PAYMENT-SIGNATURE: PaymentPayload') +
        wire(4, 'S', 'F', 'POST /verify') +
        wire(4, 'F', 'S', 'isValid: true — 온체인 실행 없음', '', true) +
        wire(5, 'S', 'F', 'POST /settle') +
        wire(5, 'F', 'C', 'transferWithAuthorization — 가스 = facilitator', '', true) +
        wire(6, 'S', 'A', '200 + 콘텐츠 + PAYMENT-RESPONSE') +
        '</div>' +
        '<div class="x402-fldline" data-f="fld"></div>' +
        '<div class="banim-status" data-f="rst"></div>' +
        x402Dialog(),
      render(step, setF, root) {
        const snd = new Set(), rcv = new Set();
        root.querySelectorAll('.banim-wire').forEach((w) => {
          const cur = +w.dataset.mstep === step;
          w.classList.toggle('cur', cur);
          w.classList.toggle('past', +w.dataset.mstep < step);
          if (cur) { snd.add(w.dataset.from); if (w.dataset.to) rcv.add(w.dataset.to); }
        });
        root.querySelectorAll('.banim-actor').forEach((a) => {
          a.classList.toggle('snd', snd.has(a.dataset.a));
          a.classList.toggle('rcv', !snd.has(a.dataset.a) && rcv.has(a.dataset.a));
        });
        setF('fld', FLD[step]);
        setF('sig',
          step >= 6 ? '<span class="cv">nonce 소진 — 같은 서명으론 재정산 불가</span>'
          : step >= 5 ? '온체인 제출됨 — 정산 확인 대기'
          : step >= 3 ? '서명 전달됨 — PAYMENT-SIGNATURE 헤더로 나감'
    ...
    // 재생 컨트롤을 캡션 바로 아래로 + 전체 JSON 모달 — x402 전용 (다른 애니메이션 무영향)
    el.classList.add('banim-x402');
    const ctl = el.querySelector('.banim-ctl');
    el.insertBefore(ctl, el.querySelector('.banim-stage'));
    const dlg = el.querySelector('.x402-dialog');
    const jbtn = document.createElement('button');
    jbtn.type = 'button';
    jbtn.textContent = '전체 JSON';
    jbtn.title = '스펙 예시를 바탕으로 재구성한 전체 필드 보기';
    ctl.appendChild(jbtn);
    jbtn.addEventListener('click', () => { if (dlg.showModal) dlg.showModal(); else dlg.setAttribute('open', ''); });
    dlg.querySelector('[data-x402-close]').addEventListener('click', () => dlg.close ? dlg.close() : dlg.removeAttribute('open'));
  ...
  return { esc, renderMarkdown, initMermaid, runMermaid, fixMermaidListLabels, enhanceDiagrams, enhanceCodeCopy, enhanceSectionRefs, enhanceToc, mountAnims };
})();
