...
한국어 문서 작성·번역·수정·검토에는 [한글 기술 문서 작성 규칙](../prompts/korean-writing.md)을 적용한다. 이 규칙의 표현·의미 보존 및 자료 정리의 근거·범위 기준은 상위 wiki의 3-layer 운영 규칙과 별개로 이 모듈에도 적용한다.
...
- `group: <묶음 이름>` (선택) — **내보낸 문서집에서 묶음 소제목으로 갈라 준다.** 칸반에는 영향 없다.
  ...
  내보낸 문서의 이전·다음 이동도 묶음 표시 순서를 따른다. `WaaS 도입·구축/Dfns`는 배포 방식·운영 환경 / 서명 보안·검증 / 구축·도입 검토로 묶는다.
...
- ★ **추측 금지** — 확인 안 된 필드·값·동작·순서를 문서에 쓰지 않는다. 원본(공식 스펙·확정 결정)에서
  확인되면 쓰고, 확인 불가면 생략하거나 "확인 필요/미확정" 절에 질문으로만 남긴다. "저자 정리"로 표시해도
  추측을 본문·비교표·요약·다이어그램에 넣지 않는다. 자료의 미기재를 기능 부재로, 특정 버전의 설명을 현재 제품 상태로 단정하지 않는다.
