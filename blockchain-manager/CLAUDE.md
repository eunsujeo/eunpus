...
- `ref: 참고` (선택) — **참고 문서 표시.** 판단 재료·심화 설명처럼 설계 본문과 붙어 읽히지만 공유 대상은 아닌 문서에 붙인다.
  칸반에는 그대로 보이고, **HTML 내보내기에서 기본 제외**된다 (`--with-ref` 로 포함). 내보낸 문서에서 빠진 문서를
...
- ★ **조립 문서 함께 갱신** — 다른 장의 결론만 모은 문서들은 원천이 바뀌면 같이 고친다:
  `BC/설계/01`(인프라 — 전체 배치·문서 구성)·`BC/설계/02`(흐름 조립)·`블록체인매니저/설계/16`(인터페이스 한 장).
  계약·경계·배치를 바꾸는 편집이면 이 셋을 확인할 것.
