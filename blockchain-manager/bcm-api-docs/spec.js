  ...
  "paths": {
    "/accounts": {
      "post": {
        ...
        "summary": "계정 생성",
        "description": "`accountType` 과 `ref` 로 vault 를 만들고 `ref ↔ accountId` 매핑을 반환한다. `ref` 는 호출 쪽 계정 ID 를 그대로 쓴다.\n고객 계정뿐 아니라 시스템(운영) 계정도 이 오퍼레이션으로 만든다. **두 유형의 ID 는 서로 다른 체계에서 발급되고 접두사가 붙지 않아 값이 겹칠 수 있으므로 `accountType` 을 함께 받는다** — `ref` 만으로는 어느 쪽 계정인지 가릴 수 없다. 매니저는 `ref` 를 불투명 유일 문자열로만 다루고 내용을 파싱해 분기하지 않는다.\n같은 (`accountType`, `ref`) 조합으로 재요청하면 같은 `accountId` 를 돌려준다 (매니저가 이 조합으로 멱등 보장).\n",
    ...
    "/accounts/{accountId}/addresses": {
      ...
      "post": {
        ...
        "description": "**한 토큰을 여러 네트워크로 받을 때** 쓴다. 토큰 하나와 네트워크 목록을 받아 네트워크마다 입금 주소를 발급한다 — USDC 를 이더리움·폴리곤·트론에서 받으려면 `token: \"USDC\"`, `networks: [\"ETHEREUM\", \"POLYGON\", \"TRON\"]` 을 보낸다. 네트워크 하나하나는 단건 발급과 결과가 같고 **네트워크마다 멱등하다**.\n\n**받을 네트워크를 정하는 것은 호출 쪽이다.** 매니저는 토큰 이름만 받아 네트워크를 스스로 채우지 않는다 — 어떤 네트워크로 받을지는 상품 결정이다. 토큰이 여러 개면 요청을 나눠 보낸다.\n\n**계정이 없으면 요청 전체가 `404`** 다. 네트워크별 실패는 미지원 네트워크·벤더 오류 같은 네트워크 단위 사유뿐이다. 부분 성공은 정상 동작이며 전체를 되돌리지 않는다 — 응답은 **요청과 같은 순서로** 네트워크별 결과를 담고 HTTP 상태는 항목 결과와 무관하게 `200` 이다. 판단은 항목별 `error` 유무로 한다. 성공분은 멱등해 같은 주소가 다시 오므로 같은 요청을 그대로 재시도할 수 있다.\n\n**벤더 호출 수는 줄지 않는다.** 네트워크마다 벤더를 한 번 부르므로 줄어드는 것은 호출 쪽과 매니저 사이의 왕복뿐이다. 한 요청 20네트워크로 제한한다 — 한 토큰이 지원되는 네트워크 수를 넉넉히 담는 크기다.\n",
    ...
    "/accounts/{accountId}/networks/{network}/tokens/{token}/balance": {
      "parameters": [
        {
          "$ref": "#/components/parameters/AccountId"
        },
  ...
  "components": {
    "parameters": {
      ...
      "Network": {
        ...
        "description": "네트워크 코드 — DB `ntwk_cd`(20자) 대응",
        "example": "ETHEREUM"
      },
      "Token": {
        "name": "token",
        ...
        "description": "토큰 심볼 — DB `tkn_smbl`(16자) 대응",
        "example": "USDC"
      }
    },
    "schemas": {
      ...
      "CreateAddressesRequest": {
        ...
        "properties": {
          "token": {
            "type": "string",
            "maxLength": 16,
            "description": "토큰 심볼 — 이 요청의 모든 네트워크에 공통",
            "example": "USDC"
          },
          "networks": {
            "type": "array",
      ...
      "AddressResult": {
        "type": "object",
        "description": "네트워크 하나의 결과 — 성공이면 `address`, 실패면 `error` 가 채워진다 (둘 중 하나만).",
        "required": [
          "network"
        ],
        "properties": {
