  ...
  "paths": {
    ...
    "/transactions/estimate-fee": {
      "post": {
        ...
        "summary": "수수료 견적",
        "description": "낮음·보통·높음 세 단계 수수료를 추정한다. 실제 수수료는 제출 시점에 정해진다.\n대납 구성에서 이 값은 우리 vault 가 낼 돈이 아니라 relay 가 낼 실비의 예측이다.\n",
    ...
    "/accounts/{accountId}/transactions": {
      ...
      "get": {
        ...
        "summary": "거래 목록 조회",
        "description": "거래 이력을 **거래 시각(createdAt) 기준·최신순**으로 조회한다. 기간(`after`/`before`)·상태로 좁히고 커서로 페이지네이션한다.\n상태 변경 실시간 감지는 이 목록이 아니라 이벤트 큐가 담당한다(매니저 내부의 lastUpdated 감지 폴링과 별개).\n",
        ...
        "parameters": [
          ...
          },
          {
            ...
            "description": "종료 시각 — 거래 시각(createdAt) 기준 (ISO 8601 UTC)",
          ...
          },
          {
            ...
            "description": "다음 페이지 커서 — 이전 응답의 `pagination.nextCursor` 를 그대로 넣는다. 불투명 토큰이라 직접 만들거나 해석하지 않는다. 첫 요청엔 생략. cursor 가 있으면 조회 조건은 토큰이 우선이라 함께 보낸 `after`/`before`·`status`·`limit` 는 무시된다.",
  ...
  "components": {
    "parameters": {
      "AccountId": {
        ...
        "description": "매니저가 돌려준 vault 핸들 (daw-core ext_acnt_id = vaultAccountId)",
        "example": "acct_01H8X"
      },
      "Asset": {
        "name": "asset",
    ...
    "schemas": {
      ...
      "Pagination": {
        ...
        "properties": {
          "nextCursor": {
            ...
            "description": "다음 페이지 커서 (불투명 토큰) — 다음 요청 `cursor` 로 그대로 전달. null 이면 마지막 페이지.",
            "example": "eyJsYXN0IjoxNzUxMzM2MDAwMDAwfQ"
          },
          "hasMore": {
            "type": "boolean",
      ...
      "Account": {
        ...
        "properties": {
          "ref": {
            "type": "string",
            "description": "우리 참조 키 (daw-core 계정 ID) — 고객 ACT-000123, 운영 SYS-000001 (역할 HOT_OPS·FEE_MGT·RESERVE)",
            "example": "ACT-000123"
          },
          "accountId": {
            "type": "string",
            "description": "매니저가 돌려주는 vault 핸들 (daw-core ext_acnt_id = vaultAccountId)",
            "example": "acct_01H8X"
          }
        },
        "required": [
      ...
      "Fee": {
        "type": "object",
        "description": "수수료 추정 한 단계. 세부(가스 단가·한도 등)는 체인별로 구현에서 정의.",
        "properties": {
          "amount": {
            "type": "string",
            "description": "예상 수수료(문자열)",
            "example": "0.0012"
          }
        },
        "required": [
      ...
      "CreateAccountRequest": {
        ...
        "properties": {
          "ref": {
            "type": "string",
            "description": "우리 참조 키 (영구 유일) — daw-core 계정 ID. 고객 `ACT-000123`, 운영(관리) `SYS-000001` (역할 HOT_OPS·FEE_MGT·RESERVE).",
            "example": "ACT-000123"
          }
        }
      },
      "FeeEstimateRequest": {
        "type": "object",
        "description": "수수료 추정에 필요한 전송 형태만. 트래블룰·externalTxId·메모는 넣지 않는다.",
        "properties": {
          "from": {
            "allOf": [
              {
