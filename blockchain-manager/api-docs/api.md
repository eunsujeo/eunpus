...
| 필드 | 타입 | 필수 | 설명 |
|---|---|---|---|
| `ref` | string | 필수 | 우리 참조 키 (영구 유일) — daw-core 계정 ID. 고객 `ACT-000123`, 운영(관리) `SYS-000001` (역할 HOT_OPS·FEE_MGT·RESERVE). |
...
| 이름 | 위치 | 타입 | 필수 | 예시 | 설명 |
|---|---|---|---|---|---|
| `accountId` | path | string | 필수 | acct_01H8X | 매니저가 돌려준 vault 핸들 (daw-core ext_acnt_id = vaultAccountId) |
...
|---|---|---|---|---|---|
| `accountId` | path | string | 필수 | acct_01H8X | 매니저가 돌려준 vault 핸들 (daw-core ext_acnt_id = vaultAccountId) |
| `after` | query | string | 필수 | 2026-07-01T00:00:00.000Z | 시작 시각 — 거래 시각(createdAt) 기준 (ISO 8601 UTC) |
| `before` | query | string | 필수 | 2026-07-13T00:00:00.000Z | 종료 시각 — 거래 시각(createdAt) 기준 (ISO 8601 UTC) |
...
| `cursor` | query | string | - | eyJsYXN0IjoxNzUxMzM2MDAwMDAwfQ | 다음 페이지 커서 — 이전 응답의 `pagination.nextCursor` 를 그대로 넣는다. 불투명 토큰이라 직접 만들거나 해석하지 않는다. 첫 요청엔 생략. cursor 가 있으면 조회 조건은 토큰이 우선이라 함께 보낸 `after`/`before`·`status`·`limit` 는 무시된다. |
...
| 필드 | 타입 | 필수 | 설명 |
|---|---|---|---|
| `nextCursor` | string \\| null | - | 다음 페이지 커서 (불투명 토큰) — 다음 요청 `cursor` 로 그대로 전달. null 이면 마지막 페이지. |
| `hasMore` | boolean | 필수 | 다음 페이지 존재 여부 |
...
| 필드 | 타입 | 필수 | 설명 |
|---|---|---|---|
| `ref` | string | 필수 | 우리 참조 키 (daw-core 계정 ID) — 고객 ACT-000123, 운영 SYS-000001 (역할 HOT_OPS·FEE_MGT·RESERVE) |
| `accountId` | string | 필수 | 매니저가 돌려주는 vault 핸들 (daw-core ext_acnt_id = vaultAccountId) |
...
| `createdAt` | string | 필수 | 거래 생성 시각 (목록 정렬·기간 필터 기준) |
| `lastUpdated` | string | 필수 | 마지막 상태 변경 시각 |
