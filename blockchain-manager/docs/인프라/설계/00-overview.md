...
구역은 사내 인프라와 외부 벤더·네트워크 둘이다 — 컴플라이언스 구성요소도 지갑 기반과 **같은 네트워크**에 있다.
...
파랑 = 직접 만들고 운영하는 서비스, 노랑 = 벤더가 강제해서 우리 인프라 안에 두는 설치물, 회색 = 외부 벤더·네트워크. 안쪽의 지갑 기반·컴플라이언스 묶음은 역할 구분일 뿐 네트워크 경계가 아니다 — 둘은 같은 네트워크에 있다.
...
| 매니저 DB | 블록체인 매니저 | 벤더 번역·운영 상태 — ref↔vault↔주소 매핑 · 이벤트 체크포인트 · finalize 트랜잭션 원본 보관([블록체인매니저 15장](../../블록체인매니저/설계/15-raw-tx-archive.md)) |
