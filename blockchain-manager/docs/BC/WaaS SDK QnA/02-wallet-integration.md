...
## 9. 트래블룰·KYC·보험 의무는 누가 지는가?

**Q.** 트래블룰 정보 전송, 고객 KYC·AML, 이용자 자산에 대한 보험·준비금 의무는 은행과 JV 중 누가 지나요? JV가 대행하는 부분이 있다면 계약과 사업자 신고 범위에 들어 있나요?

수탁 구조도 3장은 고객 계정·KYC·AML을 은행 몫으로, Fireblocks 계약·Co-Signer 운영을 JV 몫으로 적었다. 이 배분은 설계 문서 해석이라 규제상 의무 주체와 일치하는지 법무 확인이 필요하다. 트래블룰 정보를 누가 보내는지는 [컴플라이언스 게이트 설계](../설계/04-compliance-flow.md)의 호출 주체와도 연결된다.
