...
두 체인이 겹쳐 오고, 여기에 **tx당 알림 수**가 곱해진다 — 생애 2~3개(created·CONFIRMING·COMPLETED)인지 **컨펌마다**인지에 따라 순간치가 몇 배로 갈린다(미확인). 새 체인이 붙으면 그 파형이 더해진다. **정확한 순간 최댓값은 부하테스트로 확정한다.**
...
"캐시에서만 읽어 왕복 0"에는 전제가 있다 — **JWKS 를 처음(또는 키 로테이션 때) 가져오는 조회는 외부 I/O 라서, 짧은 타임아웃(수 초)을 걸고 요청 처리 스레드 밖(워커 스레드)에서 해야 한다.** 타임아웃 없이 요청 스레드에서 돌면 egress 이상 시 벤더 재시도가 올 때마다 스레드가 하나씩 잠기고, 헬스체크까지 굶어 죽어 **폭주가 아닌 평시에도 수신기가 재시작 루프에 빠진다** (2026-08 [PoC 실측](97-webhook-poc-result.md)).
...
| 원자 기록 | **한 트랜잭션**으로 `bcm_tx_l` 갱신 + `bcm_outbox_l` 에 발행 이벤트 적재 + 알림 처리 완료(`prcs_yn=Y`)를 함께 커밋 |
...
- **장치 2 — relay 가 발송, 컨슈머가 dedup.** 별도 relay 가 `bcm_outbox_l` 의 미발송(`P`) 이벤트를 집어 큐로 보내고 `S` 로 표시한다(at-least-once). 보낸 뒤 표시 전 죽어 재발송해도 컨슈머가 `evnt_id` 로 접는다 → 효과는 한 번.
...
| 워커 트랜잭션 커밋 전 | 롤백 → `prcs_yn` 그대로 N → 재처리 | 이중 없음 (원자성) |
...
step: ④ 벤더가 실패분 재전송 | 벤더가 그 알림들을 다시 보낸다 → 인박스에 적재되고 큐로 발행돼 공백이 메워진다
