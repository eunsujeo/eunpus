...
벤더 기록에는 **성공한 이동만** 들어온다. 그래서 Fireblocks 만 보면 "이번 배치로 100 이 옮겨졌다" 로 읽히고, **82 에서 200 을 옮기려다 실패했다는 사실은 어디에도 없다.** 우리가 두 건을 요청했다는 정보 자체가 벤더 쪽에 없기 때문이다.

이게 `bcm_swp_trgt` 정리에 걸린다. 82 를 sweep 대상으로 마킹해 뒀다면 그 행을 지울지 다시 시도할지 정해야 하는데, 벤더 응답에는 82 가 없으니 판단 근거가 안 나온다. 실패를 알려면 두 가지가 우리 쪽에 있어야 한다.

1. **무엇을 요청했는지** — 제출 시점의 `extraParameters.contractCallData` (알림 원문에 그대로 실려 온다)
2. **각각 어떻게 됐는지** — 영수증의 `SweepLeg` 이벤트

이 둘을 대조해야 성공·실패 집계가 선다. 벤더 레코드는 성공분 확인에만 쓴다. 알림 원문은 [배치 sweep payload 실물 샘플](94-batch-payload-sample.md)의 부분 실패 절에 있다.
