...
| **`400` (같은 `externalTxId` 중복)** | 우리가 이미 보냈다는 뜻이다 — 벤더에 `externalTxId` 로 조회해 `txId` 를 회수하고 `SUBMITTED` |
| **`400`(그 밖의 검증) · `409` · `422`** | 요청 자체가 거절된 것이 확실하다 — `FAILED` 로 두고 오류를 그대로 돌려준다. 행은 지우지 않는다(같은 키의 재사용을 계속 잡아야 한다) |
