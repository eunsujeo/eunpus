...
| 상태 전이만 outbox 적재 | 체크포인트의 마지막 발행 상태와 비교해 앞으로 가는 전이만 outbox 에 넣는다. 같으면 생략, 과거로 돌아가면 무시 |
| 원자 기록 (outbox) | bcm_tx_l 갱신·outbox 적재·처리 완료를 **한 트랜잭션**으로 — relay 가 큐로 발행(at-least-once)하고 재발송분은 아래 중복 반영 방지가 거른다 |
| 중복 반영 방지 | 같은 이벤트가 두 번 와도 한 번만 반영한다 — **이벤트 id(`evnt_id`)** 로 가린다. `txId` 로 가리면 한 tx 의 감지·확정·무효화가 같은 키가 되어 확정이 버려진다 |
| 반영 판정 | dedup 을 통과한 뒤 **허용 전이 표**(아래 상태 절)로 가린다 — 늦게 온 옛 상태는 무시, `COMPLETED → FAILED`(reorg 무효화)는 반영 |
| 발행 순서 | relay 는 **같은 계정 안에서 `evnt_id` 순으로 순차 발송**한다 — 병렬로 보내면 같은 tx 의 감지·확정이 큐에 뒤바뀐 순서로 들어간다 (파티션 키는 큐에 들어간 순서만 보장한다) |
...
알림·이벤트는 **도착 순서를 믿을 수 없다.** 우리가 200 을 못 준 알림은 벤더가 나중에 다시 보내므로, 오래된 CONFIRMING 이 이미 처리한 COMPLETED **뒤에** 도착할 수 있다(재전송 API 는 실패분을 다음 분에 몰아 보내 이 역전을 더 잘 만든다). 그래서 판단 기준은 도착 순서가 아니라 **직전 상태와의 전이가 이 표에 있는지**다.

| 직전 상태 | 새 상태 | 처리 |
|---|---|---|
| (없음) | `SUBMITTED`(출금) · `CONFIRMING`(입금) | 신규 — 발행·반영 |
| (없음) | **`COMPLETED` · `REJECTED`** | 기록을 만들고 **감지 이벤트를 합성해 먼저 발행한 뒤** 그 상태를 발행한다 — 순서 계약을 매니저가 지킨다 (아래) |
| (없음) | **`FAILED`** | 기록만 남기고 **발행하지 않는다** — 감지도 못 한 tx 가 이미 증발한 것이라 반영할 자금이 없다 |
...
- ★ **`COMPLETED → FAILED` 는 예외가 아니라 규칙이다.** 상태에 서열을 매겨 "뒤로 가면 무시"로 구현하면 **reorg 무효화가 조용히 버려지고 고객 잔액이 되돌려지지 않는다.** 무시 여부는 서열이 아니라 이 표로 판정한다.
- **`REJECTED` 는 종결이 아니다** — 입금 동결은 Admin 해제로 결과가 바뀐다(출금 `REJECTED`·`BLOCKED` 는 벤더 기준 종결).
- 이 표는 **매니저의 발행 판정과 DAW-CORE 의 반영 판정에 같이 쓴다.** 한쪽만 지키면 다른 쪽에서 같은 사고가 난다.
- **컨펌 수는 줄지 않는다** — 늦게 온 알림은 낮은 `numOfConfirmations` 를 담고 있으므로, 기록은 **큰 값으로만** 갱신한다(감소 금지). 막힘 점검이 이 값을 입력으로 쓰기 때문이다.
- ★ **이벤트 순서는 매니저가 보장한다** — 벤더 알림은 순서를 믿을 수 없지만, **DAW-CORE 가 받는 이벤트는 한 tx 에 대해 항상 `감지 → (확정 | 무효)` 순서다.** 벤더 사정을 번역해 깔끔한 계약을 주는 것이 매니저의 일이므로, 순서 정리를 소비 쪽에 넘기지 않는다.
  - **왜 필요한가** — 실패한 알림은 자기 재시도 일정으로 나중에 오고 그 사이 **뒤에 생긴 알림이 먼저 도착한다.** 그래서 `CONFIRMING` 을 못 받은 채 `COMPLETED` 가 먼저 올 수 있다. zero-confirmation 설정에서도 첫 알림이 `COMPLETED` 다(아래 확정 기준 절).
  - **어떻게 보장하나** — 발행하려는 상태의 앞 단계를 아직 발행하지 않았다면, 매니저가 **감지 이벤트를 그 알림의 내용으로 합성해 먼저 발행**한다(확정 알림에는 주소·금액·txHash 가 다 들어 있다). 두 이벤트를 **같은 트랜잭션에 outbox 로 적재**하고 relay 가 `evnt_id` 순으로 내보내므로 순서가 지켜진다.
  - **DAW-CORE 는 무엇을 믿어도 되나** — "감지를 못 받은 확정"은 오지 않는다. 감지로 대기(pending)를 만들고 확정으로 가용 전환하는 구현을 그대로 두면 된다. 다만 zero-confirmation 설정에서는 **감지와 확정이 거의 붙어 온다.**
  - 알림마다 독립적인 상태·재시도 일정을 갖는 것은 실측으로 확인됐다(재전송 API 가 알림 단위로 집계). **벤더가 전달 순서를 보장한다는 문서·확답은 없으므로** 순서를 믿지 않는 쪽으로 설계한다 — 벤더가 어느 쪽이든 이 표대로면 안전하다.
