...
- 로컬 자산 등록 BFF는 명시적 활성화, `FUNCTION_TEST`, loopback target·binding, 동일 Origin, JSON body, 전용 비단순 헤더를 모두 서버에서 검증한다. 브라우저는 BCM·Fireblocks를 직접 호출하거나 감사 헤더를 조립하지 않는다. 공유 환경에서는 mTLS·단기 JWT 경계가 완성될 때까지 route와 버튼을 닫는다.
