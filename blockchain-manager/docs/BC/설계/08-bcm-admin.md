...
- T10.2의 첫 용도는 읽기 전용 기능 테스트다. frontend·BFF와 대상 BCM은 기본적으로 loopback에 바인딩하고 BCM 운영 상태 변경 API를 BFF에 노출하지 않는다. 이후 추가한 로컬 Stub 시나리오 실행은 `FUNCTION_TEST+STUB+LOCAL+loopback`에서 고정 실행기만 여는 예외이다. 로컬 자산 매핑 관리를 명시적으로 활성화한 `FUNCTION_TEST+loopback`에서는 후보 조회·등록만 추가로 열고, 논리 해제·교체는 열지 않는다. 두 예외 모두 공유 운영 변경 경계를 넓히지 않는다.
...
- 로컬 자산 관리 BFF는 명시적 활성화, `FUNCTION_TEST`, loopback target·binding을 서버에서 검증한다. 후보 조회는 전용 비단순 헤더를, 등록은 이 헤더와 동일 Origin·JSON body를 모두 확인한다. 브라우저는 BCM·Fireblocks를 직접 호출하거나 감사 헤더를 조립하지 않는다. 공유 환경에서는 mTLS·단기 JWT 경계가 완성될 때까지 route와 버튼을 닫는다.
