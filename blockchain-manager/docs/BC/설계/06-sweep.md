...
- **핫월렛(고객 vault + 옴니버스 + 출금 풀 + Gateway 잔액)은 법정최대치 20% 를 넘을 수 없다** — 밴드 설계의 뿌리.
...
| 보내는주소들 | **옴니버스 vault + 출금 풀 vault + Gateway 잔액** | 정책의 한 계층을 우리가 셋으로 쪼갰다 |
| 콜드월렛들 | **현 설계에 없는 계층** | 후보는 아래 "콜드월렛을 무엇으로 두나" |

정책은 계층이 셋인데 우리는 vault 가 넷이다. **보내는주소 하나를 셋으로 쪼갠 것**이고, 쪼갠 이유는 규제와 무관하다 — 옴니버스는 sweep 이 모이는 보관처, 출금 풀은 EVM 의 vault 당 nonce 직렬을 피하려고 복수로 둔 제출 창구, Gateway 잔액은 체인 변환용 완충이다. 셋 다 즉시 쓸 수 있는 자산이라 핫이다.

1차 sweep 목적지는 **옴니버스 vault**다(2026-08-05). 옴니버스 → 출금 풀 → Gateway 사이의 이동은 **같은 칸 안에서의 이동이라 밴드S 판정을 바꾸지 않는다** — 어디에 얼마를 둘지의 운영 배분 문제다. 콜드 계층 이동만 별도 정책으로 다룬다.
...
| 항목 | 정의 |
|---|---|
| 총자산합 (=100%) | **고객 vault + 옴니버스 + 출금 풀 + Gateway 잔액 + 콜드월렛** 전체 이용가능잔액의 원화환산 합 (가수금 포함) |
| 핫월렛합 · 핫월렛백분율 | **고객 vault + 옴니버스 + 출금 풀 + Gateway 잔액**의 원화환산 합 · 핫월렛합×100/총자산합 |
