...
경로가 여러 단이면 **중간 단계가 Finalized 될 때까지 이동량이 실현되지 않는다.** 아래 "콜드월렛 계층 — 요건" 의 출구 최소화·지연 반영과 함께 정한다.
...
무엇으로 구현하든 이 성질을 만족해야 한다. 구현 후보는 아래 표에 있고 아직 고르지 않았다.

| 요건 | 내용 |
|---|---|
| **즉시성 없음** | 오프라인 서명이라 자동 충전이 성립하지 않는다. 하한 미달은 경보를 올리고 사람이 승인·서명한다 |
| **왕복이 비대칭** | 핫 → 콜드는 자동화할 수 있고, 콜드 → 핫은 사람을 거친다. 두 방향의 소요 시간이 다르다는 전제로 밴드 하한을 잡아야 한다 |
| **목적지 고정** | **승인된 고정 목적지로만** 나갈 수 있어야 한다. 강제 수단은 구현마다 다르다 — 외부 콜드면 TAP 화이트리스트(등록에 admin quorum 승인), 벤더 cold workspace 면 워크스페이스 간 이동이 화이트리스트인지 Network 연결인지 확인이 필요하다 |
| **출구 최소화** | 핫에서 콜드로 나가는 vault 가 늘수록 화이트리스트와 정책을 그만큼 여러 곳에 걸어야 한다. 통제점 수가 출구 수를 따라간다 |
| **지연 반영** | 경로가 여러 단이면 중간 단계가 Finalized 될 때까지 이동량이 실현되지 않는다. 그동안 밴드S 판정은 옛 값을 보므로 **같은 이동을 다시 트리거한다.** 진행 중 이동량을 예약분으로 빼거나 실행 중 잠금을 두어야 한다 |
| **총자산합 포함** | 콜드는 핫이 아니지만 분모에는 들어간다. 잔액을 어떻게 읽어 오는지가 구현마다 다르다 |

마지막 줄이 구현 선택에 걸린다 — 벤더 워크스페이스면 API 로 읽히고, 외부 콜드면 잔액을 어디서 받아 오는지 따로 정해야 한다.
