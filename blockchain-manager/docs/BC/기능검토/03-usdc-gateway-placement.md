...
**인출 목적지를 바꾼다고 잔액의 자산 경계가 바뀌거나 업무 계열이 정해지지 않는다.** 고객 옴니버스의 Gateway 잔액은 어디로 인출하든 고객자산이고, 회사자산 vault 로 보내면 경계를 넘는 이동이 된다. **경계를 넘는 이동은 업무 요청이 있어야 하고, 목적지만으로 계열을 정하지 않는다**([운영 설계](07-usdc-gateway-operating.md) 기록 규칙) — 오프램프 델타 정산이면 제출 원장 `INTERNAL`, 브릿지교환이면 교환 식별자로 두 다리를 묶는다. 업무 근거 없는 단방향 이동은 금지다. 따라서 Gateway 인출의 목적지만으로는 업무 계열을 표현할 수 없다.
