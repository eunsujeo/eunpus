...
**Smart Transfer** 는 그 위에 올라간 정산 기능이다. 정산 한 건을 티켓으로 만들고, 양쪽이 주고받을 자금을 티켓 안의 정산 항목으로 적어 이행한다.
...
**티켓**이 정산 한 건이고, **정산 항목(leg, API 에서는 term)** 은 한쪽이 상대에게 보내야 할 자금 하나다. 자산·금액·보내는 프로필·받는 프로필을 묶은 **한 건의 지급 의무**라고 보면 된다.

예를 들어 우리가 상대에게 1 BTC 를 보내고 상대가 우리에게 100,000 USDC 를 보내는 정산이라면, 한 티켓 안에 다음 두 항목이 들어간다.

1. 우리 → 상대: 1 BTC
2. 상대 → 우리: 100,000 USDC

| 단위 | 담는 것 |
|---|---|
| 티켓 | 티켓 ID · 정산 방식 · 상태 · 만든 프로필 · 만료 시각 · 외부 참조 ID · 메모 |
| 정산 항목(leg/term) | 항목 ID · 자산 · 금액 · 보내는 프로필 → 받는 프로필 |

정산 항목마다 보내는 쪽과 받는 쪽이 따로 있어서 **한 티켓에 양방향이 들어간다.** 서로 주고받는 두 지급 의무를 한 묶음으로 합의한다.
...
| 방식 | 동작 | 상태 |
|---|---|---|
| `ASYNC` | 정산 항목에 자금 출처를 지정하면 **그 항목의 자산이 즉시 전송된다** | 현재 제공 |
| `DVP` | 각 항목의 자금 출처를 지정해 **컨트랙트의 자산 이동을 승인**하고, 실제 정산 때 모든 항목을 하나의 DVP 전송으로 실행한다 | Early Access |

★ **`ASYNC` 는 원자적 교환이 아니다.** 정산 항목이 각각 독립해서 즉시 실행되므로, 우리는 보냈고 상대는 보내지 않은 상태가 성립한다. 티켓은 그 상태를 기록하고 만료시킬 뿐 되돌리지 않는다. 신용 위험이 남는다.

★ **`DVP` 에도 우리가 먼저 승인하고 상대는 아직 승인하지 않은 중간 상태는 성립한다.** 그러나 이 승인은 상대에게 자산을 보낸 것이 아니라, 컨트랙트가 정산 시 자산을 옮길 수 있도록 권한을 부여한 것이다. 실제 자산 이전은 모든 정산 항목을 하나의 DVP 전송으로 함께 실행하므로, **우리 자산만 상대에게 전달되고 상대 자산은 오지 않은 정산 결과는 성립하지 않는다.**

`DVP` 는 Early Access 라서 Fireblocks 가 워크스페이스에 활성화해 줘야 쓸 수 있고, 담당자에게 문의해야 한다. 한쪽만 승인한 동안 자산이 잠기는지, 티켓 만료 시 승인이 자동 해제되는지는 공개 문서만으로 확인되지 않아 별도 확인이 필요하다.
...
- 기관 간 정산. 양쪽의 지급 의무를 티켓 하나에 담아 합의한다.
...
| `find-ticket` | `DVP` 는 모든 정산 항목을 하나의 DVP 전송으로 실행한다 |
| `smart-transfer-webhooks` | 티켓·정산 항목 필드 · 생애주기 · 양쪽 이행 완료 시각 |
