...
- **서명 장치마다 3조각 묶음이 따로 있다.** Owner 묶음이 뿌리이고 Signer·Admin·API user 가 추가될 때마다 그 장치 전용 묶음이 같은 master seed 에서 나온다. 묶음이 여럿이어도 지갑 주소는 하나다.
...
|---|---|
| 지원 서명 | ECDSA, EdDSA |
| 조각 결합 방식 | additive secret sharing — Shamir 의 t=n 과 같고 결합은 단순 덧셈. 모든 조각 없이는 정보 이론적으로 아무 정보도 얻을 수 없다 |
| 개인키 존재 여부 | "the secret itself never exists — even during the key generation ceremony" |
| 라운드 | 4 라운드, 그중 3 라운드는 사전 처리. GG18 의 8 라운드 대비 800% 빠르다고 명시 |
| 오프라인 서명 | 마지막 라운드를 QR 로 전달할 수 있어 air-gapped 서명이 가능 (Cold Wallet) |
...
한 묶음 안에서는 3조각 전부 필요하고(3/3), 묶음이 여러 개일 때는 어느 한 묶음이 3/3 을 채우면 유효하다(1/N). 공식 문구: "None of the parties (neither Fireblocks nor the customer) can sign a transaction alone."
...
| 서명 권한 장치 합류 — Signer·Admin 모바일, API Co-signer | 그 장치 전용 조각 묶음. 같은 master seed 라 master key·지갑 주소 불변 | Owner 명시 승인 |
...
- **묶음(key share set)은 서명 장치 단위.** 공식 문구: "Admins and Signers have their own unique key share set. No two signing devices share the same key share set."
- **Owner 묶음이 뿌리.** 모든 Signer·Admin 묶음은 Owner 묶음에서 파생된다. 파생 절차의 암호학적 세부는 공식 문서에 없다.
...
- **API Co-signer.** API user 하나가 묶음 하나다. Co-signer 한 대에 API user 여러 명을 페어링하면 묶음 여러 개가 그 장비에 저장된다. Co-signer 를 여러 대 두면 장비마다 자기 API user 의 묶음을 갖는다 — 조각을 나눠 갖는 것이 아니다. 2대 구성은 [Co-signer HA 구성](../설계/12-cosigner-ha.md).
...
| 사건 | 승인 | 조각에 일어나는 일 |
|---|---|---|
| 서명 가능 사용자 추가 | 사용자 추가는 Owner + Admin Quorum, 조각 발급은 Owner 단독 (별도 승인 2건) | 새 묶음 발급 |
...
- Owner 묶음에서 다른 묶음을 파생하는 암호학적 절차·라운드·실패 처리 — 공식 문서 없음.
...
- API user 를 unpair·삭제할 때 그 묶음의 cloud 조각이 삭제되는지 — Console 사용자 삭제 경로만 확인됨.
...
| ID | 출처 | 반영 범위 |
|---|---|---|
| FB-KM-001 | [MPC-CMP](https://support.fireblocks.io/hc/en-us/articles/6984668676124-MPC-CMP) | 프로토콜·조각 결합·3/3·묶음 단위·Broker·Aggregator·안전장치 |
