...
|---|---|---|
| 온램프 대응 | **출고** — 미리 채워 둔 재고가 델타 배치로 고객 vault 로 나간다 | 고객이 받을 체인 |
...
| 인출 목적지 | 기술 | 남은 것 |
|---|---|---|
| 최종 목적지로 바로 — 출금 풀·온램프 대응 목적지 등 | **확인됨** — 다른 vault account 와 일회성 주소를 예제가 보인다([기능](02-usdc-gateway.md)) | 운영 선택 |
...
**경계를 넘는 건은 별도로 기록한다.** Gateway 인출이 출금 풀로 갈 때는 브릿지교환의 한 다리이므로 반대 다리와 짝을 이뤄 기록한다.

## 선행해서 정할 것

| | 왜 걸리나 |
|---|---|
| **회사자산 vault 의 실체** | BC 설계의 vault 역할 표는 고객 vault·옴니버스·출금 풀 셋뿐이고 회사자산 vault 가 없다. 어느 워크스페이스·누가 운영·TAP 정책이 정해져야 이 설계가 얹힌다 |
...
첫 줄이 선행이다. 회사자산 vault 는 우리 설계 문서에 정의된 적이 없다.
