...
→ 따라서 MM에 무엇이 보이나·제3자가 못 보나·DAML이 원장에서 강제하나는 국내은행 뷰만으로는 부분적이며, 노드인프라 지원(제3자 노드 뷰·raw Ledger API·패키지)이 있어야 완전 검증된다. 아래 "검증 가능성"에 표시.
...
"제한적" 항목의 완전 검증에는 다음이 필요하다(→ 6장 C·F):
