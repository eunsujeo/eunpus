...
- [ ] **저장 용량 증가율** — 거래량 대비 참여자 DB 증가 추정치나 경험칙이 있나. ACS 와 이벤트 로그의 비율, 조회용 PQS 를 포함하면 총 몇 배인가. 보존 기간을 숫자로 정하려면 이게 있어야 한다. Canton 공식 문서에는 산정 기준이 없다 ([참여자 노드 운영](../운영%20검토/00-participant-node-operations.md) 6절).
...
- [ ] **용량 실측** — PoC 시작·종료 시점에 참여자 DB 크기를 재서 거래 1건당 증가량을 낸다. 스토리지 조회 권한이 필요한지, 노드인프라가 대신 재 줄 수 있는지 확인.
