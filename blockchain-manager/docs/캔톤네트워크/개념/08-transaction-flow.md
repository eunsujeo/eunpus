...
시퀀서·미디에이터는 암호화된 거래로 순서와 확인만 다뤄 금액·상대를 못 읽고, 커밋은 즉시·결정적이라 되돌림이 없다.
...
- **밸리데이터(참여자 노드)** — 온-원장. 거래를 암호화해 순서화를 요청하고, 자기 뷰(view)를 검증·확인하고, 커밋으로 원장 상태를 갱신한다.
...
- **③** A 밸리데이터는 거래를 통째로 넘기지 않고 **암호화해서** 시퀀서에 순서화만 요청한다.
...
시퀀서와 미디에이터는 거래를 **암호화된 채로만** 다룬다. 시퀀서는 순서를 매기고 미디에이터는 확인을 세지만, 안에 든 **금액도 상대도 못 읽는다**. 순서와 확정이라는 공용 기능을 맡으면서도 거래 내용은 이해관계자에게만 남는 구조다 — 이것이 캔톤의 선택적 신뢰다(뷰 분배는 3장, 프라이버시는 9장).
...
| **밸리데이터(참여자 노드)** | 온-원장 | 거래를 암호화해 순서화를 요청하고, 자기 뷰를 검증·확인하고, 커밋으로 원장을 갱신한다. |
| **시퀀서** | Synchronizer | 전역 순서만 매겨 이해관계자에 분배한다. 암호화돼 있어 내용은 못 읽는다. |
| **미디에이터** | Synchronizer | 밸리데이터들의 확인을 취합해 확정을 통지한다. 암호화돼 있어 내용은 못 읽는다. |
