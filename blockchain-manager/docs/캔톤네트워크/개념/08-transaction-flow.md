...
거래는 A 지갑의 한 번의 요청에서 출발해, 오프-원장(지갑·앱)을 지나 A 밸리데이터에서 온-원장으로 올라간다(①②). A 밸리데이터는 거래를 통째로 넘기지 않고 **암호봉투**로 봉해 시퀀서에 순서화만 요청한다(③). 시퀀서는 전역 순서를 매겨 이해관계자인 A·B 밸리데이터에 뿌리되 **각자 자기 뷰만** 준다(④). 두 밸리데이터는 자기 뷰를 검증해 미디에이터에 "유효함"을 확인하고(⑤), 미디에이터가 그 확인을 취합해 확정 결과를 통지한다(⑥). 확정이 내려오면 양쪽 노드가 동시에 커밋해 원장 상태가 바뀌고(⑦), B 앱이 그 결과를 읽어 B 지갑에 "100 받음"을 표시하면 한 건이 끝난다(⑧).
