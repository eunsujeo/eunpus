...
권한이 함수 본문이 아니라 타입 선언에 들어가는 게 솔리디티와의 결정적 차이이고, 이 규칙은 작성 → 빌드 → 배포(vetting) → 사용 순으로 원장 위에서 동작한다.
...
## 권한이 타입 선언에 들어간다 — 솔리디티와의 차이

권한은 함수 본문이 아니라 **타입 선언에 들어간다**. 이게 솔리디티와의 결정적 차이다.
...
솔리디티는 누구나 호출할 수 있게 두고 본문에서 걸러내지만, Daml 은 타입 수준의 **선언으로 강제**한다. 규칙을 코드로 검사하는 게 아니라, 규칙 자체가 타입이 된다.
...
signatory 가 컨트랙트의 존재와 소멸을 좌우하고, observer 가 누가 볼 수 있는지를 정하고, controller 가 각 choice 의 실행 주체를 정한다. 세 선언이 곧 그 컨트랙트의 권한·가시성 규칙 전부다.
...
서명자가 리스트라 승인자 전원이 함께 컨트랙트의 권한자이고, `venue` 와 각 leg 의 tradingParties 는 observer 로 열람만 한다. `_Accept` 를 실행할 수 있는 건 그 호출의 `approver` 뿐이다 — controller 가 choice 안에 고정돼 있기 때문이다.

## 이 코드는 어떻게 배포·사용되나

템플릿을 썼다고 끝이 아니다. **작성 → 빌드 → 배포(업로드 + 승인) → 사용** 순으로 원장 위에서 동작한다. 핵심은 배포 단계다 — DAR(빌드 산출물)은 그 거래에 참여하는 **모든 이해관계자 노드**에 올라가 **vetting(승인)** 돼야 한다. 한 노드만 갖고 있으면 그 패키지로 만든 거래는 검증·커밋되지 않는다.
