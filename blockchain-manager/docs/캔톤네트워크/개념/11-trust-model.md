...
- **[안 믿어도]** 원장 무결성·일관성 — **내 밸리데이터만 정직하면** 상대는 내가 낀 무효 거래를 확정 못 시킨다. 내 노드와 상대 노드의 사본이 실제로 같은지는 [6장의 ACS 대조](06-architecture.md#노드끼리-어긋나면-어떻게-아나-acs-대조)가 주기적으로 확인한다.
...
- **Synchronizer** — 단일 운영자 vs BFT 시퀀서(노드의 2/3 초과가 정직하면 동작 — 장애·악의 노드가 1/3 미만) vs 여러 Synchronizer 연결로 단일 장애점 제거.
- **글로벌 Synchronizer** — 최대 탈중앙화: 여러 독립 슈퍼 밸리데이터 + BFT 합의(임계값까지 비잔틴 견딤) + 투명한 CIP.

전통 블록체인이 "다수가 정직하다"에 거는 신뢰를, Canton은 "주로 내 밸리데이터"로 좁히고 나머지는 암호로 강제한다.

## 정리 — 개념에서 코드까지

개념(프라이버시·원자성·신원/확정성·신뢰)과 그 코드(파티 ID·Daml·choice 시퀀스·포트·instrumentId)가 한 흐름으로 이어졌다. Canton이 강한 곳은 **기관 간(B2B) 정산**이고, 리테일·퍼블릭 DeFi엔 그 강점이 오히려 안 맞는다 — 도구는 용도에 맞는 곳에서 값을 낸다.
