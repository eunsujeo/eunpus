...
| **기둥 3** | 가시성 없는 동기화 | Synchronizer(시퀀서+미디에이터)가 **내용을 못 본 채** 순서와 확인만 맞춘다 — 암호화된 거래만 다룬다. |
...
flowchart TB
    ...
    P3["기둥 3 — 가시성 없는 동기화<br/>Synchronizer는 내용을 못 본 채<br/>순서·확인만 · 암호화된 거래만 다룸"]
...
그래서 Synchronizer는 **"믿을 수 있는 중개자"가 아니라 "믿을 필요가 없는 중개자"**다. 신뢰를 요구하는 게 아니라, 신뢰가 필요 없도록 설계된 것이다 — 내용을 못 보는 중개자는 그 내용을 이용해 거래를 조작할 수단이 없다.
