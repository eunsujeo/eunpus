...
Canton 원장은 **불변 컨트랙트**의 모음이다(eUTXO(확장 UTXO) 식). 상태를 바꾼다는 건 값을 덮어쓰는 게 아니라 **옛 컨트랙트를 보관(archive)하고 새 컨트랙트를 만드는** 일이다. 같은 컨트랙트는 한 번만 소비할 수 있으므로, 같은 자산을 두 번 쓰는 이중지불이 구조적으로 막힌다.
...
**컨트랙트 키**: 컨트랙트 ID 를 몰라도 `(은행, 계좌번호)` 같은 키로 컨트랙트를 조회한다. 키마다 그것을 책임지는 **maintainer** 가 있다(Canton 3.5 예정).
