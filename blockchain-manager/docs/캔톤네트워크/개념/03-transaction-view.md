...
앞 장의 **세 기둥**(부분 트랜잭션 프라이버시 · 이해관계자 합의 증명 · 가시성 없는 동기화)이 한 거래에서 실제로 어떻게 도는지, 처리 단계까지 펼치면 아래처럼 된다. **지금 전부 이해할 필요는 없다** — 제출·순서화·뷰 전달·확인·커밋의 각 조각은 뒤 장(8장·9장)에서 하나씩 푼다. 우선 **"같은 거래인데 노드마다 받는 게 다르다"**만 눈에 담으면 된다.
