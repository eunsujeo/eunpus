...
[흐름](?cat=온보딩&sub=설계) 02장 감지(웹훅)의 **대안 검토안 — 웹훅 수신이 불가한 환경(보안 정책 등)에 대비해 완성해 둔 설계.** 토픽 publish·큐 계약은 02장을 그대로 쓰고, 수신부만 웹훅 수신기에서 폴러로 바꾼다. 웹훅 사용이 허용되어 채택은 보류(2026-07-23) — 감지 속도(실시간 push)가 결정 사유.
...
정리하면 벤더 쪽에서 보는 요청 속도는 두 숫자다 — **평상시 초당 0.1~0.2회 · 피크 합산 최대 초당 10회**(클라이언트측 상한). 이 수치로 벤더에 정상 범위 확인을 요청했다(2026-07-23 메신저 문의 — 회신 대기, 미확정 절).
...
- **정상 호출률 확인 — 채택의 전제 조건** — 평상시 초당 0.1~0.2회 · 피크 합산 최대 초당 10회(클라이언트측 상한)가 이상 트래픽으로 볼 수준이 아닌지. 공식 Rate Limiting 문서가 "웹훅 대신 폴링으로 다수 tx 모니터링"을 rate limit 을 유발하는 흔한 실수로 들고 있어서 이 확인 없이는 확정하지 않는다 — 다만 그 실수는 tx 1건당 호출 1번인 패턴이고, 이 설계는 목록 조회라 500건당 1번이다 — 5분 내 생성 500건(하루 14만 건 페이스)까지는 물량과 무관하게 주기당 1번. 이 수치로 벤더에 문의 발송(2026-07-23 메신저) — 회신 대기. 부정적이면 스캔 주기 완화(30~60초) → 웹훅 재검토 순. 429 대응은 문서 권고대로 지수 백오프.
