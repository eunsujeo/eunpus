...
**컴플라이언스 서비스 — 규제 확인의 솔루션 연동 전담.** 출금·입금의 트래블룰 확인을 요청하는 창구이자, 상대 VASP 발 사전 검증 기록을 보관·대조하는 곳이다. 솔루션(VerifyVASP·CODE·Notabene) 원어를 공통 어휘(TrVerdict)로 번역한다 — 월렛은 어느 솔루션인지 모른다. 트래블룰 게이트가 첫 모듈이고, **AML 스크리닝(Chainalysis·Elliptic)·OFAC 제재 주소 차단이 다음 모듈 후보다** — 지금은 벤더(Fireblocks) 안 별개 통합으로 남는다([컴플라이언스 0장](../../컴플라이언스/설계/00-scope.md)).
...
| 블록체인 매니저 | API Co-signer + Callback Handler | 보안 존(SGX/TEE) 설치물 — 키 share 공동서명 · 서명 직전 승인·거부 | [블록체인매니저 6장](../../블록체인매니저/설계/06-withdrawal.md) |
| 컴플라이언스 | 컴플라이언스 서비스 | 규제 확인 전담 — 라우터 + 솔루션 어댑터. 트래블룰 게이트가 첫 모듈. 1차 대조(사전 검증 기록 — 컴플라이언스 DB)는 이 서비스, 귀속·가용 전이는 월렛 백엔드 | [트래블룰 8장](../../트래블룰/설계/08-gate-port.md) · [12장](../../트래블룰/설계/12-physical-layout.md) |
| 컴플라이언스 | VerifyVASP Enclave | 벤더 강제 설치물 — 키·PII · PUBLIC HTTPS | [트래블룰 12장](../../트래블룰/설계/12-physical-layout.md) |
...
| 저장소 | 소유 | 담는 진실 |
|---|---|---|
| 백엔드 DB | 두 백엔드 | 회계 진실 — 고객 원장·귀속·출금 지시 상태 · VASP 마스터(거래 허용·정체). 벤더가 바뀌어도 남는다 |
| 매니저 DB | 블록체인 매니저 | 벤더 번역·운영 상태 — ref↔vault↔주소 매핑 · 폴링 커서 · 거래 추적 · finalize 원본. 테이블은 [블록체인매니저 17장](../../블록체인매니저/설계/17-database.md) |
| 컴플라이언스 DB | 컴플라이언스 서비스 | VASP 레지스트리(솔루션 목록·`cmpl_vasp_id`·코어 `vasp_id` 매핑·활성화) · check 상태 · 사전 검증 기록(입금 도착 시 대조로 답한다). 거래 허용·VASP 정체는 백엔드 VASP 마스터 몫. 테이블은 [컴플라이언스 2장](../../컴플라이언스/설계/02-database.md) |
