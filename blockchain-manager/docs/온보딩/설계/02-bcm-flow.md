...
| 규칙 | 내용 |
|---|---|
| 서명 검증 | 모든 알림은 Fireblocks 공개키로 서명을 검증하고 통과한 것만 받는다 |
| 수신 확인 | 2xx 를 돌려줘야 전달 완료 — 아니면 벤더가 재시도한다 |
| 유실 복구 | 재시도로도 못 받은 알림은 재전송 API(`resend_failed`)로 다시 받는다. 수신기 재기동 후 1회 호출한다 |
...
- **웹훅 서명 검증 스펙·페이로드 구성** — 키 배포·헤더 형식, 알림만으로 판정이 끝나는지(단건 보강 조회 필요 여부) — 벤더 확인 후 확정.
- **재전송(resend) 보장 기간·실패 판정 기준** — 수신기 다운 허용 시간·대사 주기의 근거 — 벤더 확인 후 확정.
