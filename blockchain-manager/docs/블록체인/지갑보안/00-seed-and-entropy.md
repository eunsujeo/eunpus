...
난수 생성기를 통틀어 RNG(Random Number Generator)라 하고, 만드는 방식에 따라 둘로 갈린다.

| | TRNG (하드웨어 진짜 난수) | PRNG (의사난수) |
|---|---|---|
| 출처 | 물리 현상 — 전기 잡음·열 잡음 등 | **시작값 하나로 계산해 만든 수열** |
| 예측 | 물리적으로 불가능 | **시작값을 알면 전부 재현 가능** |
| 하드웨어 지갑 | 지갑 시드를 만들 때 이걸 쓴다 | 암호용 PRNG(CSPRNG)라도 시작값의 엔트로피가 부족하면 무의미하다 |
...
1. **엔트로피를 뽑는다** — 12단어면 128비트.
2. **체크섬을 붙인다** — 엔트로피를 SHA-256 에 넣고, 결과의 앞 (엔트로피 길이 ÷ 32) 비트를 뒤에 이어 붙인다. 128비트면 4비트가 붙어 132비트가 된다.
3. **11비트씩 자른다** — 132 ÷ 11 = 12조각. 조각 하나가 단어 하나가 된다.
4. **목록에서 그 번호의 단어를 읽는다** — 목록은 2048개로 고정이라 11비트가 표현하는 범위(0~2047)와 정확히 맞는다.
