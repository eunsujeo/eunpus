...
역할을 가르면 — **위임장의 내용과 집행 함수는 EIP-3009** 가 (무엇을·누구에게·언제까지, 그리고 `transferWithAuthorization`), **위임장의 서명 서식은 EIP-712** 가 (구조화 서명 — 지갑이 "0.01 USDC 를 0x2096… 에게" 처럼 내용을 표시할 수 있다), **서명된 위임장을 체인에 올리는 창구는 facilitator** 가 맡는다.
...
|---|---|---|
| 클라이언트가 서명하는 것 | **오프체인 authorization** (EIP-3009 위임장, EIP-712 서식) — 트랜잭션이 아니다 | **온체인 트랜잭션 자체를 구성해 부분 서명** — fee payer 서명 자리가 비어 있다 |
...
정산 상태는 셋으로 다룬다. ① **제출·확인 대기** — 브로드캐스트 후 확인이 불명확하면 facilitator 가 `settlement_pending` 과 트랜잭션 해시를 반환하고, 호출자는 그 해시로 온체인 상태를 대조한다. 이 상태에서는 정산도 nonce 소진도 확정이 아니다. ② **성공 확인** — nonce 소진, 같은 authorization 으로 재정산 불가. ③ **실패 확인** — 이때에만 새 authorization 을 만든다. 성공 여부가 불명한 채 새 nonce 로 서명하면 그것은 새로운 유효 결제가 되어 이중 지불이다.
...
출처: [x402 v2 스펙 보관본](https://github.com/eunsujeo/eunpus/blob/main/sources/stablecoin/markdown/2026-08-20__github-com__x402-specification-v2.md) · [저장소 README 보관본](https://github.com/eunsujeo/eunpus/blob/main/sources/stablecoin/markdown/2026-08-20__github-com__coinbase-x402.md). 필드 정밀 인용은 [원문 스펙 (기준 고정본)](https://github.com/x402-foundation/x402/blob/7d5363a6d51750dc246041f2b0ed5819dd46a0d7/specs/x402-specification-v2.md)을 본다.
