...
지불 측 관리형 구성은 AWS 와 **선택한** 지갑 인프라 업체(Coinbase CDP 또는 Stripe/Privy) 가 맡고 받는 쪽(merchant 의 402 처리·facilitator 정산 대행)은 담당 업체가 정해져 있지 않다 — 이 대비는 팀의 해석이다.
...
Payments 의 PaymentSession 기본 통제는 금액·통화·만료 중심이고 merchant·asset 별 결제 정책식은 이번에 수집한 AWS AgentCore 공개 문서에서는 확인하지 못했다 (AgentCore 의 다른 구성요소에 별도 통제가 있을 수 있다 — Payments 밖은 이 문서의 범위가 아니다). 우리 시스템 기준으로 보면, Fireblocks Transaction Policy 의 First-Match rule·quorum 승인과는 다른 접근이다 — **표현력을 쌓는 대신 노출 자체(충전액·세션 한도)를 줄이는 모델**이다. 이 대비는 팀의 해석이고, 두 제품의 공식 문서가 서로를 비교한 것은 아니다.

비용 관점에서도 같은 경계가 나온다 (팀 관점): 자동 호출 빈도가 높아질 수 있어 건당 소액도 누적이 빠르고 "살 가치가 있는가" 판단을 에이전트가 하므로 오판이 자동으로 반복될 수 있다. 실제로 지출을 제한하는 것은 충전액과 세션 한도다 — 단, 세션 한도는 세션 생성(관리) 권한과 실행 권한이 분리된 경우에만 지출 제한으로 작동한다 (위 IAM 분리). AWS 가 밝힌 사실은 마이크로페이먼트를 첫 단계로 택했다는 것까지고, 그 범위 설정이 이 위험을 줄이는 효과가 있다는 것은 팀 해석이다. 고액 커머스 확장(AWS 로드맵)에는 더 촘촘한 정책 표현력이 필요해 보인다. **facilitator/sponsor 의 수수료·정산 모델은 이번에 수집한 공개 문서에서는 확인하지 못했다** — 대납된 가스·대행 수수료가 누구에게 어떻게 청구되는지가 총비용을 가른다 ([가스 대납 문서](../가스대납/00-overview.md)의 "대납은 수수료를 없애지 않는다"와 같은 질문이다).
