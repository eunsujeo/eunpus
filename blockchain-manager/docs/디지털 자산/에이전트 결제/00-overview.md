...
에이전트 결제는 AI 에이전트가 HTTP 요청 과정에서 유료 자원(API·MCP 서버·웹 콘텐츠)의 대가를 사람 개입 없이 지불하는 구조다. 결제 수단의 중심은 스테이블코인이고 — x402 v2 는 asset 에 법정화폐 통화 코드(ISO 4217)도 허용하며 AWS 는 법정화폐 지원을 로드맵에 두고 있다 — 전통 결제 시스템이 감당하지 못하는 소액(센트 단위 이하) 마이크로트랜잭션이 첫 용도다.

구체 예로: 리서치 에이전트가 유료 시세 API 를 호출하면, 서버가 데이터 대신 HTTP `402 Payment Required` 와 "이 응답은 0.01 USDC" 라는 가격표를 돌려준다. 에이전트는 지갑으로 결제 증명에 서명한 뒤, **실패했던 그 요청을 결제 증명 헤더만 더해 그대로 다시 보내고**(재시도), 이번에는 데이터를 받는다. 사전 충전·권한 부여(지갑 hub 화면 — 아래 역할 표)를 마친 뒤에는, 개별 결제가 별도 화면 없이 이 왕복 안에서 끝난다 — 402 는 HTTP 표준에 예약만 돼 있던 상태 코드이고, 이 프로토콜들이 그 자리를 실제 결제에 사용한다.
