...
카드사가 블록체인을 직접 운영하지 않으면서 스테이블코인 결제·정산을 수행하는 분업이다. 우리 시스템에서 Service 백엔드가 업무 원장·정책·고객 책임을, [블록체인 매니저](../../블록체인매니저/개요/00-overview.md)가 온체인 실행·번역을 맡는 분리와 같은 구조다 — 이 대응은 팀의 해석이다.
...
- 온체인 AML·FDS 가 결제 흐름의 어느 지점에서 실행되는지

이 항목들은 wiki open question 으로 추적 중이고 세미나·발표 자료에서 확인되면 갱신한다. 세션에서 물을 질문은 [세미나 확인 질문](01-seminar-questions.md)에 모았다.
