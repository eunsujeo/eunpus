...
핵심 결정은 트래블룰 게이트를 블록체인 매니저 밖, 별도 컴플라이언스 서비스에 둔다는 것이다.

- **블록체인 매니저**는 vault·서명·온체인 감지/전파를 맡는다. 즉 "자산을 실제로 움직이는" 부분이다.
- **컴플라이언스 서비스와 DAW-CORE**는 그 앞뒤에서 트래블룰 솔루션 왕복(서비스)과 대조·성명 매칭·입금대기 판정(DAW-CORE)을 맡는다. 즉 "움직여도 되는가를 판단하는" 부분이다.
...
- **트래블룰(신원 교환)은 지갑/VASP 층이 맡는다** — 컴플라이언스 서비스의 VerifyVASP 연동과 DAW-CORE의 매칭·귀속 판단이 여기에 해당한다.
