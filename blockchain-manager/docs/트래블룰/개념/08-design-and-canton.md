...
- **입금 = 대조 실패 시 잔고 차단** — 국내 VASP 정보 또는 해외 VASP·개인지갑의 위험기반 확인이 부족한 입금은 금액과 관계없이 입금대기(잔고 차단·락업)로 묶는다.
...
핵심 결정은 트래블룰 게이트를 블록체인 매니저 밖의 별도 서비스 — 컴플라이언스 게이트 — 에 둔다는 것이다.

- **블록체인 매니저**는 vault·서명·온체인 감지/전파를 맡는다. 즉 "자산을 실제로 움직이는" 부분이다.
- **컴플라이언스 게이트와 DAW-CORE**는 그 앞뒤에서 트래블룰 솔루션 왕복(서비스)과 대조·성명 매칭·입금대기 판정(DAW-CORE)을 맡는다. 즉 "움직여도 되는가를 판단하는" 부분이다.

이 분리는 벤더 지형과도 맞물린다. Fireblocks의 공식 트래블룰 제공자 목록은 Notabene(직접)·Sumsub·GTR(TRLink)·Chainalysis·Elliptic이고, 국내에서 쓰는 **VerifyVASP는 이 목록에 없다**. 국내 솔루션을 쓰는 순간 게이트는 벤더가 대신 처리해 주는 기능이 아니라 우리가 벤더 밖에서 직접 세워야 하는 관문이 된다. 게이트를 매니저 밖 별도 서비스로 두는 설계는 이 현실과 자연스럽게 들어맞는다.
...
flowchart TB
    subgraph OUT["출금 경로"]
      ...
      OBM["블록체인 매니저<br/>제출 — 서명·전파는 벤더 안"]
      OG -->|Match 통과| OBM
...
- **트래블룰(신원 교환)은 지갑/VASP 층이 맡는다** — 컴플라이언스 게이트의 VerifyVASP 연동과 DAW-CORE의 매칭·귀속 판단이 여기에 해당한다.
...
- **실제 구현 설계는 [트래블룰 설계 세트](../설계/00-screening-order.md)** — 검사 순서·Notabene 연동·게이트 배치(포트-어댑터)·국내(VerifyVASP·CODE) 병행·시나리오·물리 배치·국내 연동 준비까지, 이 장의 두 게이트가 실제로 어떻게 세워지는지의 정본이다.
