...
- **대기함은 월렛 백엔드가 보관** — 입금 대조의 재료(7.3·7.5).
- **스크리닝 호출은 자산 경로 밖** — `validate` 계열은 제출·전파를 타지 않고 전용 경로로 나간다(8장).
- **자격증명은 코드·설정 밖** — 전용 API user(스크리닝 권한만)의 키는 시크릿 스토어/KMS 에 두고, 서비스 안 스크리닝 클라이언트가 로드한다. 커스터디 등 다른 서비스의 자격증명과 분리.
