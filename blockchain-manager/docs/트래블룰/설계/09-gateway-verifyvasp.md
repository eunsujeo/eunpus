...
검증에 통과하면 6장이 택한 국내 직접 연동(자체 Enclave·수신 콜백)을 덜거나 없앨 수 있다 — 모든 상대가 해외 벤더주도 흐름(7.2/7.4) 한 벌로 처리되기 때문이다.
...
이 모델에서 VerifyVASP 의 **비동기 승인 왕복은 사라지지 않고 자리를 옮긴다** — 직접 연동(7.1)에선 제출 전 우리 게이트가 기다리지만, 여기선 제출 뒤 **벤더 게이트 안의 대기(`Pending`/Wait)** 로 들어간다. 그래서 상대 승인(사람 심사 포함)이 벤더의 차단 시간(Outbound delay 최대 90분·Wait 4시간) 안에 끝나야 성립하고, 늦으면 `Blocking Time Expired` 로 거래가 취소된다 — 아래 체크리스트 ③이 이 조건의 확인이다.
...
7.3(직접 연동 입금)이 요구하던 **우리 수신 사슬(Enclave → 컴플라이언스 게이트 수신 콜백)이 사라지고**, 7.4 처럼 웹훅으로 결과만 받는다. 송신측의 VerifyVASP 사전 통지는 게이트웨이가 받아 두었다가 도착한 입금과 대조한다.

## 직접 연동(7.1/7.3)과의 대비

| | 게이트웨이 경유 (이 장) | 직접 연동 (7.1/7.3 · 6장 경로 B) |
|---|---|---|
| 우리 인프라 | 없음 — Fireblocks+Notabene 만 | 자체 Enclave · 수신 콜백 |
...
- **라이브·검증 통과 시** → 국내 직접 연동(자체 Enclave·수신 콜백, 6장 경로 B)을 제거·축소. 국내·해외가 7.2/7.4 한 벌로 합쳐진다.
