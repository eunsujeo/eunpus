---
title: 9. 게이트웨이 경유 VerifyVASP — 검증 대상
status: To Do
---

VerifyVASP 회원(국내 거래소)에 **자체 Enclave 없이 Notabene 게이트웨이로 우회 도달**할 수 있는지를 검증하는 후보다. 확정 설계가 아니라 검증 대상이며, **Notabene 의 VerifyVASP 라이브 지원 여부 자체가 공개 자료로 확정되지 않는다**(공개 분석 페이지는 "보안 검토 대기"라 하나 노후·상충) — 벤더 확인이 선결이다.
...
7.3(직접 연동 입금)이 요구하던 **우리 수신 사슬(Enclave → 수신 컴포넌트 → 내부 API)이 사라지고**, 7.4 처럼 폴링으로 결과만 받는다. 송신측의 VerifyVASP 사전 통지는 게이트웨이가 받아 대조 재료로 삼는다.
...
- **① 라이브 지원 여부(선결)** — Notabene 의 VerifyVASP 통합이 실제 라이브인지 **벤더 확인**부터. 공개 자료는 "보안 검토 대기"라 하나 노후·상충한다. 라이브 확인 후 **한국 VerifyVASP 회원(예: 업비트)까지 실제로 닿는지** 별도 확인(목록 등재 ≠ 도달).
- **② 사전 허가형 정합** — VerifyVASP 는 송신측이 자금 전 우리 승인을 기다리는 왕복이다. 이 핸드셰이크가 게이트웨이 경유로 온전히 오가나 — **특히 입금 방향**.
...
- **주장·상태** — Notabene [SafeGateway](https://notabene.id/solutions/safe-gateway) · [VerifyVASP 분석 페이지](https://notabene.id/travel-rule-messaging-protocols/verifyvasp)(공개 웹). 후자는 "보안 검토 대기"라 적으나 **노후된 페이지로 보이며**, 라이브 상태는 벤더 확인 필요.
- **흐름** — 위 주장을 우리 스택(7.2/7.4)에 대입한 **설계 판단**. 회색 박스 내부 동작은 미검증.
- **미확정** — ①~④ 는 벤더 확인 전까지 확정 아님. 6장 미확정 "게이트웨이 경유 VerifyVASP 도달"과 같은 항목이다.

## 관련 장

- 확정된 국내 직접 연동: 7.1/7.3, 6장(경로 B·Enclave 요건).
- 형태가 같은 해외 벤더주도 흐름: 7.2/7.4.
- Enclave 필요/불필요 판단·게이트 포트: 8장.
- 도달 못 하는 상대(GTR 단독 등)의 차단·입금대기: 6장 게이트, 7.5 합류점.
