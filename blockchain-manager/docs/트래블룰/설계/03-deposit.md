...
벤더 안 흐름에는 우리 코드가 없다 — Fireblocks 와 Notabene 가 검사·판정을 안에서 끝내고, 우리는 웹훅으로 결과 상태만 받는다. 도착한 뒤의 가용 전이는 별개다 — 7.5 합류점의 판별(벤더 통과를 어디까지 믿을지의 정책 결정 포함)이 우리 몫이다.
...
## 입금 한 건이 벤더 안을 지나는 길 — 우리는 웹훅으로 결과만 받는다
...
sequenceDiagram
    ...
    FB->>PW: 웹훅 — 동결이면 REJECTED 계열 상태로 나타난다
    Note over PW: 이후는 기존 입금 처리 그대로 —<br/>동결 건은 잔액 미반영 · Admin unfreeze 로 해제
```

벤더 안에서 감지·스크리닝·판정·조치가 끝나고, 우리는 결과 상태만 받는다. 그림의 "우리 웹훅 수신"은 **블록체인 매니저의 웹훅 수신**(블록체인매니저 설계 4장)을 접은 표현이다 — 백엔드는 매니저가 큐에 publish 한 이벤트를 consume 할 뿐이고, 내부 분담이 펼쳐진 그림은 7.4 시나리오다.

## 이후 — 기존 입금 파이프라인이 그대로 흡수한다

입금 트래블룰은 벤더 안에서 끝나고, 우리 쪽에는 웹훅이 받는 상태(REJECTED·동결)로만 나타난다. 따라서 별도 처리 코드를 새로 짜지 않는다.
...
시간 규칙은 하나 짚어 둔다. Inbound delay(기본 30초) 안에 판정이 나지 않으면 기본 설정에서는 통과(자금 방출) 된다.
