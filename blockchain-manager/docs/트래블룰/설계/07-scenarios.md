...
- **담당자 회의의 입금 원칙**도 이 흐름과 같다 — 자금 도착은 통제할 수 없으므로 출처 확인 전 잔고 가용화를 보류한다. 다만 회의에서 언급된 TXID 기반 상대 조회는 공개 VerifyVASP 명세에서 확인되지 않았으므로, 정확한 API가 확인될 때까지 위 그림처럼 `verificationUuid` 기반 사전 기록·상태 조회를 구현 기준으로 둔다.
