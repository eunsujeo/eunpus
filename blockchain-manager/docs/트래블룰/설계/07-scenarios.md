...
국내·해외·개인지갑 × 출금·입금을 8장 공통 단계 위에서 정리한다. 입금은 도착 후 공통 판별(7.5 합류점)에서 모으고, Fireblocks 미경유·Notabene 직접 대안은 7.6·7.7, 개인지갑(자기수탁)은 7.8·7.9, 국내 CODE 직접은 7.10·7.11 에 둔다.
나누는 기준은 하나다 — **채널 밖 행위자와의 왕복이 흐름의 본체면 따로 그리고, 로컬 확인이면 접는다**. 국내(VerifyVASP)와 해외(Notabene·Fireblocks)는 상대 VASP·벤더와의 교환이 흐름의 본체라 출금·입금 모두 따로 그리고, 개인지갑은 로컬 확인이라 왕복이 없지만 완전성을 위해 따로 둔다.

## 전체 시나리오 지도

도달 가능한 상대 × 방향을 한눈에. 입금 시나리오(7.3·7.4·7.7·7.9)는 모두 확정 후 **7.5 도착 후 판별**로 합류한다.

표는 **상대별 논리 순서**로 읽고, 섹션 번호(7.1–7.11)는 추가 순서를 따른 **안정 ID** 로 본다 — 새 시나리오는 참조가 깨지지 않게 번호를 밀지 않고 뒤에 붙이므로, 국내 CODE(7.10/7.11)는 개념상 VerifyVASP 옆이나 번호는 뒤에 온다.

| 상대 유형 | 출금 | 입금 | 채널 |
|---|---|---|---|
| 국내 VerifyVASP (업비트 등) | 7.1 | 7.3 | VerifyVASP 직접 연동(자체 Enclave) |
| 국내 CODE (빗썸·코인원·코빗) | 7.10 | 7.11 | 기본 VerifyVASP 상호연동 경유(7.1/7.3 흐름 · 상세 부록 A) · 대안 CODE 직접(CODE-Cipher, 7.10/7.11) |
...
요청-응답형이다 — 자금이 오기 전에 우리 수신 사슬이 먼저 응답하고, 그 기록(대기함)이 도착 후 판별(7.5)의 대조 재료가 된다. 인바운드 사슬은 **중앙 서버 → 우리 Enclave → 수신 컴포넌트 → 백엔드 내부 API** 다(8장).
...
- 승인이 나야 상대가 온체인 전송을 실행한다 — 수신 사슬(**우리 Enclave → 수신 컴포넌트 → 백엔드 내부 API**)이 응답을 못 하면 국내 입금 자체가 막히므로, 이 사슬의 가용성이 곧 입금 가용성이다.
...
## 7.7 입금 ← 해외 (Notabene 직접) — 확정은 매니저, 대조는 우리 게이트

7.4 는 벤더가 감지·동결까지 했지만, 여기선 **벤더가 동결해 주지 않는다**. 온체인 확정은 블록체인 매니저가, 컴플라이언스 대조·입금대기(잔고 차단)는 우리 Service 게이트가 맡는다.
...
sequenceDiagram
    ...
    box rgb(224,242,254) 우리 측
    participant GT as 트래블룰 게이트<br/>수신 웹훅
...
상대가 VASP 가 아니라 이용자 본인의 자기수탁 지갑이다. 주고받을 상대가 없으니 IVMS101 교환 대신 **화이트리스트(Address Registry) 등록·소유 인증**으로 가른다. 로컬 확인이라 "트래블룰 확인 중"을 즉시 통과한다.
...
sequenceDiagram
    ...
    alt 등록 주소
        ...
        AR-->>BE: 없음 → 입금대기(잔고 차단)
        Note over BE: 개념 5장 미등록 개인지갑 발 — 등록 유도·소명·반환 정책
    end
```
...
sequenceDiagram
    ...
    alt 대기함 대조 일치
        ...
        BE->>CV: Search VASP by TXID (비동기)
        CV-->>BE: 송신 VASP 식별
        BE->>CV: Asset Transfer Data Request — 사후 정보 교환
        Note over BE: anonymous tx 절차(공식) — 대조되면 가용, 아니면 보류
    end
```
