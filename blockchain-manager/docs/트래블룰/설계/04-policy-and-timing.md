...
### 임계값 정책은 전건 처리로 전환 가능하게 둔다

VerifyVASP 담당자 회의에서는 한국의 100만원 미만 면제가 FATF 취지와 다르고, 분할 송금·시세 변동·상대 VASP 미확인 문제를 만든다는 견해가 제시됐다. 이는 현행 법령의 확정 해석이 아니므로 현재 100만원 기준을 이 발언만으로 바꾸지 않는다.

다만 설계는 임계값을 코드에 고정하지 않는다. 관할권·상대 VASP·정책 버전에 따라 `100만원 이상`과 `전건 정보 교환`을 설정으로 전환할 수 있어야 하며, 기준 미만 거래도 상대 확인·감사 기록을 남기는 정책을 선택할 수 있게 한다. 정책 변경 시 적용 근거·환산 시점·환율 출처·판정값을 함께 버전으로 보관한다.
