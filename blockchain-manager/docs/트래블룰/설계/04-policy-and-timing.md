...
제품은 VASP(가상자산사업자) 간 실제 메시지 전송이 없는 세 경우(임계값 미만·non-custodial·동일 VASP 내부)를 **Saved** 로 묶어 기록만 남긴다. 이는 제품 상태 설명이며 국내 법상 자동 면제를 뜻하지 않는다. 2027년 2월 19일 이후 국내 VASP 간 `BELOW_THRESHOLD`는 전건 정보 교환 정책으로 덮어쓴다. 이 7종이 우리 공통 어휘(TrVerdict 4종)로 어떻게 접히는지는 8장 세 어휘 대응표에 있다.
...
| 기준 미만 출금의 취급 | 벤더 기본은 `BELOW_THRESHOLD` | 국내 VASP 간 거래는 2027-02-19부터 전건 정보 교환이므로 자동 통과 금지. 다른 관할권은 적용 정책으로 재판정 |

### 2027년 전건 처리 전환

대통령령 제36592호에 따라 국내 VASP 간 정보제공 기준은 2027년 2월 19일부터 `100만원 이상`에서 `모든 이전`으로 바뀐다. 이 전환은 벤더 설정이 아니라 회사 정책 계층에서 강제한다.

- 전환 시각은 `2027-02-19 00:00 KST`로 버전 관리한다.
- 전환 전 거래 재처리 시에는 최초 판정 당시 정책 버전을 유지한다.
- 국내 VASP에서 `BELOW_THRESHOLD`가 오면 자동 승인하지 않고 정보 교환 경로로 다시 보낸다.
- 관할권·상대 유형별 정책은 코드 상수가 아니라 데이터로 관리한다.
- 금액 기준 폐지 후에도 해외 VASP·개인지갑 고액 거래 감시를 위해 환산 금액·시각·가격 출처를 보존한다.
