...
여기에 더해 기본값 표 하나가 규제 태도를 결정한다 — 그중 Skip on failure 한 줄이 "제공자가 죽으면 검사 없이 흘려보낼 것인가"를 정한다.
...
VASP(가상자산사업자) 간 실제 메시지 전송이 없는 세 경우(임계값 미만·non-custodial·동일 VASP 내부)는 **Saved** 로 묶여 기록만 남는다.

규칙이 first-match 라는 점이 안전망이다 — custom 정책을 위에 올려도 **default 규칙이 맨 아래 그대로 남아**, 어떤 거래도 규칙에 걸리지 않고 빠져나가지 못한다.

## 시간 규칙과 기본값

정책이 "무엇을·어떻게"라면, 여기 기본값 표는 "언제까지·장애 때 어떻게"를 정한다. 특히 첫 줄 하나가 규제 태도를 좌우한다.
...
기본값 **On** 은 "제공자가 죽으면 검사 없이 흘려보낸다"는 뜻이다. 규제 요구가 엄격하면 **Off 가 맞다** — 그러나 그 대가는 장애 중 출금 실패·입금 동결이다. 어느 쪽을 택할지는 규제 강도와 서비스 연속성 사이의 트레이드오프이며, **이는 설계 판단이다.**
