...
- **초기 정보 없음** — 판정에 필요한 기초 정보가 없는 거래 (벤더 용어 — 정확한 범위는 확인 필요 · 14장).
...
규칙이 first-match 라는 점이 안전장치다 — custom 정책을 위에 올려도 **default 규칙이 맨 아래 그대로 남아**, 어떤 거래도 규칙에 걸리지 않고 빠져나가지 못한다.
...
| 사전 검증 기록 보존 기간 | — | 사전 검증만 오고 자금이 안 온 기록을 언제까지 대조에 쓸 수 있게 유지할지 (7.3 사전 검증 기록) |
...
| 기준 미만 출금의 취급 | `BELOW_THRESHOLD` = 통과 | 한국 기준(원화 100만원) 미만이라도 받는 쪽 나라 기준이 더 엄격하면 상대 쪽에서 자금이 동결될 수 있다(Notabene 명세 경고 · 2장) — 그대로 통과시킬지, 받는 쪽 기준까지 확인할지 |
