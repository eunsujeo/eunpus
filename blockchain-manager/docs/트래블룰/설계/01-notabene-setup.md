...
해외(Notabene) 연동 준비를 사전 준비부터 테스트까지 정리한다 — 단일 법인 기준 5단계, 다중 법인은 VASP 매핑이 하나 더 끼어 6단계다.
Console 연결과 정책은 코드 0줄의 Admin 운영 작업이고, 개발이 손대는 곳은 출금 경로 하나(2장)뿐이다 — 단 이는 Notabene 한정이며, 국내(VerifyVASP)는 Enclave·수신 컴포넌트를 따로 만든다(13장).

## 성격 — 대부분 운영, 개발은 출금 한 곳

연동은 크게 **운영 작업**과 **개발 작업**으로 갈린다.

- **운영(Admin) 작업** — Console 연결·암호화 키 등록·VASP(가상자산사업자) 매핑·정책 설정·테스트. **애플리케이션 코드는 0줄**이고, 대부분 Fireblocks Console 과 Notabene 대시보드 조작 + 일회성 CLI/API 호출로 끝난다.
- **개발 작업** — 실제로 코드가 붙는 곳은 **출금 경로 하나**다. 이 통합은 2장에서 다룬다.

즉 이 장의 5단계는 **연동을 켜는 준비**이고, 런타임 동작 구현은 아니다.
...
## 다중 법인일 때 — VASP 매핑 규칙 (3단계)

여러 법인을 한 워크스페이스에서 운영하면 3단계에서 vault 와 VASP 를 어떻게 묶느냐가 **입금 검사의 판정 기준**을 좌우한다. 규칙은 다음과 같다.
