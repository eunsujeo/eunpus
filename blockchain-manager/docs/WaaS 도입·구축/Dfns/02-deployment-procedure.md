...
**번들은 네 계층 전부를 세운다.** 번들에 든 Terraform root 4개와 Helm chart 가 L1 기반 자원, L2 공통 기반, L3 애플리케이션 배포, L4 서명 계층을 모두 담당한다. 인프라를 만드는 데서 끝나지 않고 제품이 뜨는 데까지 쓰는 설치 키트다. 계층별로 무엇이 생기는지는 [아키텍처의 네 계층](00-on-premise-deployment.md#아키텍처의-네-계층)에 있다.
