---
title: Dfns 사내 데이터센터 구성안 — 채택하지 않음
status: 아카이브
ref: 참고
date: 2026-09-22
---

2026-09-22 에 AWS 배치로 확정되면서 채택하지 않은 안이다. 사내 데이터센터에 Dfns 전체 플랫폼을 올리는 구성을 검토한 기록으로 남긴다.
현행 설계는 같은 폴더의 Baseline 인프라 설계 문서를 본다.

지우지 않고 두는 이유는 둘이다. 첫째, AWS 구성안이 왜 그렇게 생겼는지는 이 안과 대조할 때 가장 빨리 드러난다 — 랙 3개 대 AZ 3개, Shamir 수동 unseal 대 KMS auto-unseal, 자체 운영 데이터 서비스 대 관리형 서비스가 각각 짝이다. 둘째, 배치 장소는 한 번 뒤집힌 적이 있다.

여기 적힌 자원량·망 분리·복구 목표는 **고객 설계 제안**이며 벤더가 확정한 값이 아니다. 현행 설계는 [Baseline 인프라 설계](03-baseline-datacenter-design.md) 다.
