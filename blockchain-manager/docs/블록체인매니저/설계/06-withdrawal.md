...
온체인의 발신 vault(옴니버스·출금 pool)·키(MPC)·제출자(relay)는 전부 수탁자 쪽입니다. 위임·실행 메커니즘은 [가스 대납 — 승인·계정 실행 모델](../../디지털%20자산/가스대납/02-authorization-and-account-models.md)에 정리한다.
...
| **요청 유효성** | 대응하는 출금 요청이 존재하고, 승인 완료 상태(트래블룰 확인 통과 포함 — 트래블룰 설계 2·8장)이며, 만료되지 않았는가 |
| **소비 상태** | 이 externalTxId 로 **이미 서명이 만들어지지 않았는가** — 승인된 지시 1건에 서명 1번. 이미 소비된 키의 재질의는 deny |
...
서명 관문이 강제하는 범위는 온체인 서명까지다 — co-signer 를 지나는 거래는 관문이 물리적으로 막을 수 있지만, 백엔드 내부 DB 처리·외부 API 호출은 관문 밖이라 권한 분리·감사·대사가 담당한다.
