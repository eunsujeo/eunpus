...
```kotlin
fun estimateFee(request: FeeEstimateRequest): FeeEstimate {   // 매니저 내부 — from·to·network·symbol·amount 만
...
| 부르는 시점 | 하는 일 |
|---|---|
| **주기 관측** — 매니저 내부 주기 작업 | 견적을 **시계열로 기록**한다. 우리 거래는 스테이블코인 전송으로 균질해서 견적이 건이 아니라 **시간의 함수**다 — 건마다 부르면 같은 시세를 중복 기록할 뿐이다. 제출 건에는 **제출 시각으로 그때 시세를 대응**시키고, 시세가 비정상적으로 튀는 시간대의 감지도 이 시계열 대비로 한다. |
...
월말 인보이스의 gas 실비는 **온체인 실측과 정산용 USD 환산 근거**를 함께 확인해 대사한다. 견적 시계열은 예측 대비 실측의 편차를 추적하는 보조 자료다.

2026-09-14 전달된 [타인의 담당자 Q&A](../../../sources/fireblocks-support/2026-09-14__gasless-feeinfo-third-party-qna.md)에 따르면 거래의 `feeUSD`는 **표시 전용**이다. 거래 생성 시점의 캐시 환율과 확정 시점의 가스 사용량을 사용하며, 월 청구 절차에서는 이 필드를 사용하지 않는다. 따라서 `feeUSD` 합계로 인보이스를 검증하지 않는다. 정산용 환율의 출처·기준 시각은 별도 확인이 필요하다.

대납 모델과 EIP-7702는 [디지털 자산 가스 대납](../../디지털%20자산/가스대납/00-overview.md), 채택 방식과 막힘 시 인상 재전송은 [블록체인 매니저 가스 대납 운영](../가스대납/02-state-failures-monitoring.md)을 따른다.
