...
sequenceDiagram
    ...
    App->>App: 풀 재고 확인 (풀 잔액 − 미정산(PENDING·처리 중) ≥ 요청?)
    ...
    App->>DL: 윈도 PENDING 조회 → 처리 중 마킹 (배치 키 = externalTxId) → 자산별 상계(netting)
    App->>BM: API — 자산당 순증분 1건 submitTransaction (externalTxId = 배치 키) · (고객 자산 지갑 ↔ 은행 자산 지갑)
    alt 성공 — TxRef 응답
        BM-->>App: TxRef — 배치에 기록 · 처리 중 유지, 확정 이벤트 대기
    else 확정 에러 — 검증 실패 등 · 제출 안 된 게 확실
        BM-->>App: 에러
        App->>DL: 처리 중 → PENDING 복귀 (다음 배치에 다시) · 반복 실패면 경보
...
- 확인은 두 겹 — **고객 가용**(DB 조회 — 오프램프·스왑에서 고객이 내놓는 코인)이 없는 돈 거래를, **풀 재고**(실제 풀 잔액 − 미정산(PENDING·처리 중))가 풀 부족 초과 거래를 막는다.
...
- **처리 중 마킹이 이중 제출을 막는다** — 전송 확정 전에 다음 윈도가 돌아도 PENDING 만 집으므로 안 겹친다. 재시작 시엔 "처리 중 + 배치 키" 행으로 벤더 제출 여부를 확인해 잇는다 — 애매한 에러(타임아웃)와 같은 복구 경로다.
...
- **매니저 층** — estimateFee API(7장)는 **변동 네트워크 실비**를 추정할 뿐, 실비를 미리 고정하지 않습니다.
