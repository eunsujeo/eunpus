...
회계 진실(고객 원장·귀속·출금 지시 상태)은 여기 없다 — 그것은 백엔드 DB 다([0장](00-cast.md) "DB 를 둘로 나눈 이유").
...
| 갱신 규칙 | **한 사이클의 처리·publish 가 성공한 뒤에만 저장한다.** 실패하면 저장하지 않는다 — 다음 폴이 같은 구간을 다시 받고, 중복은 멱등이 흡수한다(4장) |
...
- **약어 검수** — `bcm`·`vndr`·`vlt`·`crsr`·`dtct`·`pub` 등 새 축약어는 월렛 DB 약어집과 대조 후 확정.
