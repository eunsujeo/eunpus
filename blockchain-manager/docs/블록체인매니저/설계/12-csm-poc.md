...
**Q8. 웹훅 알림의 서명 검증 스펙(키 배포·헤더 형식)은 무엇이고, 알림 페이로드에 트랜잭션 객체(상태·subStatus·컨펌 수·주소)가 그대로 실립니까?** (4장)
> PUBLIC 수신 엔드포인트의 위조 방어 근거이자, 알림만으로 판정을 끝낼 수 있는지(단건 보강 조회 필요 여부)를 가르는 근거다.

**Q9. 재전송(resend)의 보장 기간과 "실패한 알림" 판정 기준은 무엇입니까? 수신기 다운 구간을 얼마까지 메울 수 있습니까? (v1/v2 차이 포함)** (4장)
> 수신기 다운의 허용 시간과 대사 주기를 정하는 근거다 — 보장 기간을 넘긴 유실은 대사만 잡는다.
