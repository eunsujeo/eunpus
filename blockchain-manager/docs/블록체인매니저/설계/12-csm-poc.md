...
| 정합 항목 | 합의할 것 | 관련 |
|---|---|---|
| 상태 — [4장 "공통 상태 다섯 (TxStatus)" 기준](04-detect-confirm.md#공통-상태-다섯-txstatus-기준) 그대로 합의 | 다섯 상태의 뜻·보장하는 subStatus 값·세 필드(status/subStatus/networkStatus)의 분담이 기준 표에 있다 — boost 는 매니저가 원 txId 로 접으므로 계약에 없다(6장) | 4·5·6장 |
...
| 승인 레코드 ↔ externalTxId | **1:1 대응·만료·소비 상태**를 검증 복제본에서 판정할 수 있어야 한다 — 서명 직전 검증(6장 표)의 판정 근거 컬럼 합의 | 6장 |
