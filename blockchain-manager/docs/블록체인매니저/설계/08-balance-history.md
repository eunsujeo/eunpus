...
|  | 대사 |
|---|---|
| **무엇을** | ① **DB 원장의 고객 잔액 합계 vs 온체인 커스터디 총합**(옴니버스 + 나머지 vault — 미sweep 고객 vault·출금 풀) — 옴니버스 모델의 대사 공식. **★ 델타 미정산분 조정이 미정이라 이대로 비교하면 그만큼 어긋난다**(아래). 나머지 vault 계상의 정밀 형태는 정산 워크스루 ② 거래 기록 vs 벤더 거래 목록 (표본은 온체인 탐색기로 교차 확인) |
...
flowchart LR
    ...
    C{"대조 —<br/>고객 잔액 합계 = 온체인 커스터디?<br/>델타 조정 여부 미정"}
    OK["일치 — 회계 기표 진행"]
...
- **★ 델타 미정산분 조정이 빠져 있다.** 온램프·오프램프는 원장을 먼저 반영하고 온체인 이동은 델타 배치로 나중에 하므로, 그 사이 구간에는 좌우변이 그만큼 어긋난다. 온램프는 고객 몫이 회사자산 쪽에, 오프램프는 회사 몫이 고객자산 쪽에 있어 방향이 반대다. **델타원장 순잔액을 좌변 조정항으로 두면 닫힌다** — `고객 잔액 합계 − 델타원장 순잔액 = 온체인 커스터디`. 대사를 언제 돌리느냐와 함께 정한다. 같은 조정항이 [밴드S 산식](../../BC/설계/06-sweep.md)에도 걸린다.
