...
**매니저 DB — 벤더 번역·운영 상태.** vaultId·주소 매핑과 폴링 커서를 담는다. 매니저가 이걸 쥐는 이유는 셋이다.

- 백엔드는 vaultId 를 몰라야 한다. 그래야 벤더나 매니저를 바꿔도 백엔드는 손대지 않는다(9장).
- 가장 자주 불리는 조회(depositAddressOf — 고객이 입금 화면을 열 때마다)를 벤더 왕복 없이 처리한다. 매니저가 자기 DB 만 읽는다(3장).
- 유일성을 영구 보장한다. 벤더 멱등키는 24시간 뒤 만료되지만, DB 의 UNIQUE 제약은 그 뒤에도 중복을 막는다(1·2장).

**백엔드 DB — 회계 진실.** customer_ledger·귀속·잔액을 담는다. 벤더가 바뀌어도 그대로 남아야 하는 진실이라 매니저와 분리한다.
