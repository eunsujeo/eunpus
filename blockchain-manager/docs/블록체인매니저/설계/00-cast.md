...
위는 **DAW-CORE** — 항상 직접 만들고 여기서는 **물리적으로 분리된 두 개**입니다. 별도 배포·권한·감사 경계로 나뉩니다.

- **Service 백엔드** — 고객 런타임(계정·주소·입금·출금·잔액)
- **Admin 백엔드** — 운영·거버넌스(정책·승인·키 운영·동결·rebalance)

아래는 **블록체인 매니저** — 별도 배포되는 독립 서비스입니다. DAW-CORE가 **HTTP API** 로 부르고, 온체인 이벤트는 매니저가 **메시지 큐에 publish** 하고 백엔드가 **consume** 합니다.
...
위가 논리 구조라면, 구성 요소가 실제로 **어디서 도는지**도 잡아 두면 1~8페이지가 쉽습니다. Fireblocks 기준 배치는 이렇습니다 — 서명·키·노드·전파는 **벤더 안**이고 이쪽엔 **DAW-CORE·블록체인 매니저(별도 서비스)·DB 둘·Co-signer**가 남습니다.
...
**DAW-CORE DB — 회계 진실.** 고객 원장·귀속·잔액을 담는다. 벤더가 바뀌어도 그대로 남아야 하는 진실이라 매니저와 분리한다.
