...
감지는 **Fireblocks 웹훅**입니다. 벤더가 tx 상태 변경마다 매니저의 수신 엔드포인트로 서명된 알림을 push 하고, 매니저가 계열을 갈라 판정해 **입금은 `deposit-events`(5장), 외부 출금은 `withdrawal-events`(6장), 내부 이체(delta)는 `internal-events`(10장)**로 publish 합니다. 폴링은 두지 않습니다 — 유실은 벤더 재시도·재전송(resend)이 메우고, 남는 어긋남은 대사(8장)가 닫습니다.
...
| 빠뜨리면 사고 나는 지점 | 어떻게 |
|---|---|
| **서명 검증** | 모든 알림은 Fireblocks 공개키로 서명을 검증하고 통과한 것만 받는다 — PUBLIC 엔드포인트라 위조 방어의 1차 관문. 검증 스펙(키 배포·헤더 형식)은 벤더 확인 항목(12장). |
| **수신 확인과 벤더 재시도** | 2xx 를 돌려줘야 전달 완료다 — 아니면 벤더가 재시도한다. 그래서 수신은 가볍게(검증 → 적재 → 200) 하고 판정·publish 는 분리해 처리한다 — publish 지연·큐 장애가 수신 실패로 번지지 않는다. |
| **유실 복구 (resend)** | 재시도로도 못 받은 알림은 재전송 API(`POST /v1/webhooks/{id}/notifications/resend_failed` — v2 는 일정 기간 내)로 다시 받는다. 수신기 다운·재기동 뒤에 한 번 호출해 다운 구간을 메운다. 보장 기간·실패 판정 기준은 벤더 확인 항목(12장). |
