...
- **라우팅은 매니저가 (publish 시점).** 업무 의도는 모르고, **발신자가 우리 vault 인지**로 방향을 가른다(source·dest 를 매핑과 대조):
  ...
  - **내부 이동은 destination 을 `VAULT_ACCOUNT` 로 지정해 제출한다.** 주소 문자열로 보내면 Fireblocks 가 자기 주소를 vault 로 인식하지 않아, 받는 vault 에 source 가 External 인 입금 거래가 따로 생기고 위 규칙대로면 `DEPOSIT` 으로 오발행된다(2026-09-04 실측, [vault 간 이동 PoC](../../BC/설계/92-vault-to-vault-poc-result.md)). 2차 방어로 `sourceAddress` 가 우리 vault 주소면 `DEPOSIT` 을 발행하지 않고 운영 격리 채널로 보낸다.
...
- **이벤트 순서는 매니저가 보장한다** — 한 tx 에 대해 백엔드가 받는 순서는 항상 **감지 → (확정 | 무효)** 다. 벤더 알림은 순서가 뒤바뀌어 도착할 수 있지만(실패한 알림이 자기 재시도 일정으로 늦게 오므로), 매니저가 앞 단계를 아직 발행하지 않았으면 **감지 이벤트를 합성해 먼저 내보낸다.** 그래서 백엔드는 "감지 없이 확정만 오는" 경우를 다루지 않아도 된다 — 감지로 대기(pending)를 만들고 확정으로 가용 전환하는 구현을 그대로 두면 된다. 순서 규칙 전체는 흐름 문서 허용 전이 표.
