...
sequenceDiagram
    ...
    SUB->>MDB: 커서 저장 — 처리를 끝낸 묶음의 마지막 lastUpdated<br/>오름차순 처리를 묶음(예: 페이지 크기)으로 나눠, 묶음 성공마다 저장
    Note over SUB,MDB: 실패한 묶음부터는 커서를 저장하지 않는다 → 다음 폴이 그 구간만 다시 받는다(멱등이라 중복 무해)<br/>입금 폭주 때 재수신 폭이 사이클 전체가 아니라 실패한 묶음 이후로 한정된다<br/>매니저의 역할은 여기까지 — 큐 이후(consume·원장 반영·오프셋 커밋)는 백엔드 몫 (규칙은 위 목록)
```
...
| **입금 폭주** | 오름차순 정렬(계정별 순서 보존)은 유지한 채 **처리를 묶음으로 나눠 묶음 성공마다 커서를 저장** — 사이클 후반 실패가 사이클 전체 재수신이 되지 않는다. 묶음 안 tx 의 상태 조회·기록은 배치로 묶는다. 사이클 길이·커서 정체는 경보 대상(11장). |
