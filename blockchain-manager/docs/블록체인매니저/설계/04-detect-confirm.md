...
**막힘은 이 셋 중 어디에도 싣지 않습니다.** 막힘은 새 거래 종류가 아니라 **오래 CONFIRMING 인 tx 의 상태**라 `EventType` 이 아니고, 막힘 점검이 내는 **별도 경보**입니다(막힘 점검 절). 원장·정산 컨슈머가 소비할 이벤트가 아니라 사람·운영이 받는 신호라 별도 경보 채널로 흘리고, 수단은 열어 둡니다. 막힌 tx 가 boost 로 되살아나 다시 움직이면, 그 상태 변경은 정상대로 위 데이터 토픽에 실려 옵니다.

**토픽을 나눈 이유는 소비 쪽입니다.** 한 토픽에 다 실으면 컨슈머가 매 이벤트를 입금·출금·내부 이체로 분기해야 하지만, 토픽을 가르면 각 컨슈머가 자기 계열만 맡습니다(입금 원장 반영 · 출금 상태 추적 · 정산 닫기). 세 토픽 모두 파티션 키가 계정 단위라, 같은 계정의 이벤트는 순서가 보장되어 감지 → 확정이 뒤집히지 않습니다.

이 페이지는 그 세 토픽을 채우는 **매니저 내부 폴링과 확정 기준**을 정의하고, 입금(5장)·출금(6장)·내부 이체 정산(10장)이 각 토픽을 소비합니다. 등록은 토픽별로 한 번(백엔드가 부팅 시 handler 등록), publish 는 매니저가 판정할 때마다 합니다.
...
**UNMAPPED 은 "남의 주소"가 아닙니다.** 매니저 폴링은 워크스페이스가 소유한 주소의 tx 만 벤더로부터 받으므로, 우리 것이 아닌 주소는 애초에 안 보입니다. UNMAPPED 은 **벤더엔 있으나 매니저 DB 매핑(vault↔고객)에는 없는** 수신입니다 — Console/Admin 이 매니저를 거치지 않고 만든 vault, 벤더·vault 교체 후 남은 옛 주소(9장), 주소 발급과 DB 반영 사이의 경합. 돈은 우리 vault 에 도착했는데 어느 고객인지 못 붙이는 상태라, 잔액 반영은 보류하고 vaultId 앞으로 기록해 대사로 붙입니다.
