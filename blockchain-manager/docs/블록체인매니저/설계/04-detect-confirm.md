...
sequenceDiagram
    ...
    loop 판단 — 적재된 tx 각각
        ...
        WH->>MDB: 발행 상태 체크포인트 기록 — publish 성공 후에만<br/>기록 전에 죽으면 재발행(중복 — 소비 멱등이 흡수), 반대 순서면 유실
    end
...
| **순서 역전·중복 수신** | 알림은 중복되거나 순서가 어긋날 수 있다 — 체크포인트의 마지막 발행 상태와 비교해 **상태 전이가 앞으로 갈 때만 publish** 한다(같으면 생략 · 과거로 돌아가면 무시). |
| **중복 억제 (publish 시점)** | 그래도 중복이 0 은 안 된다 — publish 성공 직후 죽어 체크포인트를 저장 못 한 창에서 한 번 더 나갈 수 있다(큐와 DB 를 원자적으로 못 묶는다). 소비 멱등이 흡수한다. |
...
### 폭주 설계 — 수신은 3步 고정, 나머지는 전부 뒤로

도착 속도는 벤더가 정한다. EVM 입금 통지는 채굴 시점에 생성되므로 **블록 단위로 뭉쳐 온다** — 10분 1만 건이면 평균은 초당 ~17건이지만, 실제로는 이더 기준 12초마다 블록당 ~200건이 한꺼번에 오고 tx 당 알림이 2~3개(created · status.updated)라 순간치는 그 2~3배다. 설계·부하 테스트 기준은 평균이 아니라 이 뭉치다: **"12초마다 알림 500건이 2초 안에 몰리는 파형 지속"** — 검증 항목은 p99 응답(벤더 응답 타임아웃이 비공개라 보수적으로 1초 이내 목표) · 오류율(circuit breaker 감안 ~0) · 적체 소화 속도.

**수신부 — 요청당 하는 일 3步 고정**

| 步 | 내용 |
|---|---|
| 서명 검증 | JWKS 공개키는 **캐시에서만** 읽는다 — 요청 경로에서 네트워크 조회 금지, 모르는 kid 일 때만 갱신. 실패면 거절 + 위조 시도 메트릭 |
| 수신 적재 | 알림 원본 insert — **키 = 웹훅 알림 id unique.** 웹훅 알림 id 는 벤더가 알림 한 발마다 붙이는 UUID(v2 페이로드의 `id`) — 같은 알림이 두 번 배달되면 같은 id 라 충돌 → 무시하고 200. 중복 방어가 물리 제약으로 끝난다 |
| 200 | insert 성공 즉시 돌려준다. **insert 실패(DB 문제)면 500** — 벤더 재시도(합산 ~8시간)가 버퍼가 되어 준다. 로컬 스풀 같은 이중 장치는 두지 않는다 |

요청당 비용이 서명 검증 + insert 1회(수 ms)로 고정이라 뭉치가 수신을 눕히지 못하고, **즉시 2xx 가 유지되므로 circuit breaker 에 걸리지 않는다** — 수신/판단 분리의 목적이 이것이다. 구현 조건 둘: 수신 경로의 DB 커넥션 풀은 판단·다른 경로와 분리한다(폭주가 수신 커넥션을 굶기지 못하게), 핸들러에 publish·매핑 조회 등 3步 밖의 일을 넣지 않는다.

**판단부 — 병렬 워커**

수신 테이블(`bcm_noti_l` — 17장)의 미처리 알림을 배치로 집어 판단(계열·전이) → publish → 체크포인트 → 처리 완료 마킹. 알림 순서가 미보장(공식 문서)이라 순서 의존이 없고 — 전이 비교(앞으로 가는 전이만 발행)가 흡수한다 — **워커를 병렬로 늘릴 수 있다.** 같은 tx 의 동시 처리만 tx 단위 잠금으로 직렬화한다. 폭주의 대가는 판단 적체(지연)뿐이고, 적체 깊이·최고 대기 시간이 경보 지표다(11장).

**구독 이벤트 (v2 카탈로그)** — `transaction.created` · `transaction.status.updated` · `transaction.approval_status.updated` · `transaction.alert.stuck_confirming` 4종으로 확정. `vault_account.asset.balance_updated` 는 보류 — 확정 즉시 sweep 제출이라면 벤더 장부 계상 전 제출을 막는 용도로 필요했겠지만, sweep 이 주기 배치(제출 직전 잔액 조회)라 필요 없다(5장).

**정지·장애 시나리오** — 복구가 전부 "재기동 + 재전송 + (경우에 따라) 재활성화"로 수렴한다. 벤더 재시도 합산 ~8시간이 기본 버퍼:

| 시나리오 | 그동안 벌어지는 일 | 복구 | 유실 |
|---|---|---|---|
| 판단 워커 정지 (수신 정상) | 200 유지 — circuit breaker 안전 · 적체만 증가 | 재기동 → 미처리분부터 소화 | 없음 — 가장 안전한 고장 |
| 수신부 정지 ≤ ~8시간 (배포·짧은 점검 포함) | 벤더 재시도가 대기해 준다 | 재기동 — 재시도가 이어서 들어옴 + `resend_failed` 1회 호출 | 없음 |
| 정지 > ~8시간 | 재시도 소진 → failed 마킹 | 재전송 API 로 회수 — **30일 내** | 없음 (30일 내 복구 전제) |
| DB 만 다운 (수신은 500 응답 지속) | 벤더 재시도 + **오류율 상승 → circuit breaker 로 웹훅 자동 비활성화 위험** | 복구 runbook: 웹훅 활성 상태 확인 → 재활성화 → 재전송 | 없음 — 재활성화를 빠뜨리면 조용한 감지 정지 |
| circuit breaker 발동 | 웹훅이 꺼진 채 오류가 안 보인다 — 조용한 정지 | "마지막 수신 시각" 메트릭(11장) + 벤더 콘솔 확인 → 재활성화 + 재전송 | 없음 (감지가 늦으면 지연 확대) |
| 큐 장애 | 수신 무관(분리 덕) — 판단만 publish 실패, 알림은 적재돼 있음 | 큐 복구 → 재처리 | 없음 |
| 벤더 장애·점검 | 알림 자체가 안 옴 — 우리 쪽 오류 0 인데 감지 공백 | 벤더 status 모니터링(11장)이 잡는다. 장애 구간 알림의 재전송 대상 여부는 미확인 → **대사가 최종 안전망** | 대사 전제 |
...
주기(운영 설정값 — 10분에서 시작)마다 벤더 거래 목록을 읽어 기록과 대조한다:

```
GET /v1/transactions?after={대조 범위 시작}&limit=500 — next-page 커서로 전부 받는다
응답의 tx 각각을 기록(bcm_tx_l)과 비교:
  기록에 없다         →  놓친 감지 — 웹훅 알림과 같은 판단 경로로 흘린다 (분류 → publish → 체크포인트)
  기록보다 앞선 상태   →  놓친 전이 — 같은 경로로 publish
  일치               →  넘어간다 — 평상시 전부 이 케이스 ("0건 확인"으로 끝)
```

- **대조 범위** = 고정 되돌아보기(예: 최근 1시간) 와 **마지막 성공 대사 시각** 중 이른 쪽부터 — 대사 자체가 오래 죽어도 재기동하면 공백 없이 이어붙는다. `after` 가 생성 시각(createdAt) 기준 필터라 가능하다(담당자 확답).
- **판단 함수를 웹훅과 공유한다** — 대사가 잡은 건도 전이 비교(앞으로 가는 전이만 발행)·소비 멱등을 그대로 지나므로 중복이 없다.
- **복구와 알림을 둘 다 한다** — 잡은 누락은 자동 복구하고, 발견 즉시 **운영 알림**도 올린다(실행당 묶음 1건 + 건수 메트릭). 누락 건수가 0에서 벗어난 것 자체가 웹훅 경로 이상(circuit breaker 발동 등)의 조기 신호다.
- **재전송과의 분담** — 재전송은 "벤더가 만들었는데 우리가 못 받은 알림"만 돌려준다. 벤더 장애로 알림 **생성 자체가 안 된** 구간은 tx 원본을 대조하는 이 대사만 잡는다 — 그래서 이것이 최종 안전망이다. tx 단위 재전송 API 는 특정 tx 재처리용 운영 도구로만 쓴다.
- 호출량은 주기당 목록 조회 1~수 회 — rate limit 영향이 없다. 방법 자체는 폴링 검토안([99. 감지 — 폴링 대체 설계](99-polling-detection.md))의 스캔을 저빈도로 돌리는 것이라, after 필터·커서·429 대응 규칙을 그대로 쓴다.
- `status=COMPLETED` 로 거를 땐 `numOfConfirmations` 가 확정 임계 이상인지 함께 본다(아래 확정 기준 절의 zero-confirmation 함정).
- 재전송·대사가 닫는 구간에서는 감지가 늦을 수 있다 — **유실이 아니라 지연**이고, 지연 폭은 재전송 호출 시점·대사 주기가 정한다.
