---
title: 4. Gateway 배치 — 자산 경계별 전용 vault 에 활성화
status: To Do
---

Gateway 지갑은 vault account 하나에 결속되고 잔액도 vault 단위다([3장](03-usdc-gateway.md)). 그래서 어느 vault 에 붙이느냐가 구조를 가른다.

**채택 — 자산 경계마다 전용 vault 하나에만 활성화한다.** 고객자산 Gateway 는 옴니버스 vault 에 붙이고 고객별 vault 에는 붙이지 않는다. 회사자산 Gateway 는 회사자산 vault 에 따로 붙인다.
...
옴니버스와 출금 풀은 다른 vault 다. 출금은 출금 풀 vault 에서 나간다([흐름](../../BC/설계/02-bcm-flow.md) 출금 절). 밴드S 는 옴니버스와 출금 풀을 한 칸으로 세므로 둘 사이 이동에 안 움직이지만, **밴드C 는 출금 풀만 세므로 출금 풀로 들어오는 이동에 늘어난다**([5장](05-fit.md)).
...
| 승인 거래 수 | **실제로 입금하는 체인 수만큼** | **활성화한 고객 vault × 실제 입금 체인의 조합 수만큼** |
...
- **승인 거래는 실제로 입금하는 체인 수만큼**이다. 옴니버스 주소가 그 체인에서 처음 Gateway 입금을 낼 때 한 번씩.
- **자동 입금 임계는 옴니버스 하나에만** 설정한다.

## 아직 정할 것

이 배치를 전제로 남는 미결은 [5장](05-fit.md)에 모았다 — 밴드C 상한·하한 값, 복수 출금 풀 배분, 감지 구분, 운영 영역 분리.
