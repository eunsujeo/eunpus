---
title: 4. Gateway 를 어디에 붙이나 — 주소 단위로 본 두 구성
status: To Do
---

Gateway 지갑은 vault account 하나에 결속되고 잔액도 vault 단위다([3장](03-usdc-gateway.md)). 그래서 **어느 vault 에 붙이느냐**가 구조를 가른다. 아직 정하지 않았다.
...
- **어느 구성인지.** 위 표만 보면 구성 A 가 단순하다. 구성 B 를 택할 이유가 있는지는 고객별로 Gateway 를 나눠야 하는 요건이 있느냐에 달렸다.
- **Gateway 잔액을 20% 한도 계산에서 뭘로 세나.** 자금이 Circle 컨트랙트에 있어 우리 vault 잔액이 아니다. 핫월렛으로 볼지 콜드로 볼지 별도 항목으로 볼지가 정해지지 않았다. 구성과 무관하게 걸리는 문제다.
