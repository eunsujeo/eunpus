...
제출 전에 한 번 읽어 보는 동사. 대납(Universal Gasless) 구성에서 이 값은 "우리가 낼 돈"이 아니라 "relay 가 낼 실비"의 예측이다.
...
sequenceDiagram
    ...
    FB-->>BE: low · medium · high (보장 아님 — 실제 수수료는 제출 때 재확정)
...
| 부르는 시점 | 하는 일 |
|---|---|
| **주기 관측** — 예: 폴링과 같은 주기 | 견적을 **시계열로 기록**한다. 우리 거래는 스테이블코인 전송으로 균질해서 견적이 건이 아니라 **시간의 함수**다 — 건마다 부르면 같은 시세를 중복 기록할 뿐이다. 제출 건에는 **제출 시각으로 그때 시세를 대응**시키고, 시세가 비정상적으로 튀는 시간대의 감지도 이 시계열 대비로 한다. |
