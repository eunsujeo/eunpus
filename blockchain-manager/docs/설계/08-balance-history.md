...
온체인 지갑(옴니버스·재고 풀 등)을 조회할 때 매니저는 `getVaultAccountAsset` 응답 필드를 세 칸으로 접어 돌려줍니다:
...
매니저 API 조회는 온체인 지갑의 잔액(1~4)과 온체인 내역(5~8)을 나른다. 벤더가 입·출금을 다 기록하므로 온체인 사건의 수집은 이 한 곳이면 된다 — 단 **고객별 귀속은 백엔드 DB 몫**이다. 매니저가 하는 일은 벤더 필드를 세 칸으로 **접는 것**과 벤더 상태(CONFIRMING/COMPLETED)를 5·6페이지의 공통 어휘로 **번역하는 것**뿐이다.
...
잔액·내역의 숫자는 결국 **두 장부**에서 나옵니다. 하나는 기록(백엔드 DB), 다른 하나는 바깥의 진실인 **Fireblocks 의 값**입니다. 매니저 내부 폴링 누락·큐 consume 지연·이쪽 반영 버그는 생기게 마련이라, 회계가 걸리는 숫자는 **주기적으로 둘을 직접 대조**합니다. 이 대사는 큐 경로와 무관하게 도는 **독립적 안전장치**입니다.
...
대사 한 사이클. 옴니버스 모델의 공식은 **"DB 원장의 고객 잔액 합계 = 옴니버스 온체인 잔액"**이다 — 두 장부가 서로 독립적으로 쌓이므로(하나는 큐에서 consume 한 이벤트 반영, 하나는 체인), 일치가 곧 매니저 내부 폴링·큐 publish/consume·백엔드 반영 경로 전체의 건강 증명이 된다. 불일치면 자동으로 고치지 않는다 — 정정도 반드시 이벤트로 남겨 사람이 판단한다.
