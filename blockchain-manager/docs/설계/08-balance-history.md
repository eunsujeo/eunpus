...
잔액·내역의 숫자는 결국 **두 장부**에서 나옵니다. 하나는 기록(백엔드 DB), 다른 하나는 바깥의 진실인 **Fireblocks 의 값**입니다. 매니저 내부 폴링 누락·웹소켓 이벤트 지연·이쪽 반영 버그는 생기게 마련이라, 회계가 걸리는 숫자는 **주기적으로 둘을 직접 대조**합니다.
...
대사 한 사이클. 옴니버스 모델의 공식은 **"DB 원장의 고객 잔액 합계 = 옴니버스 온체인 잔액"**이다 — 두 장부가 서로 독립적으로 쌓이므로(하나는 매니저 웹소켓 이벤트 반영, 하나는 체인), 일치가 곧 매니저 내부 폴링·웹소켓 push·백엔드 반영 경로 전체의 건강 증명이 된다. 불일치면 자동으로 고치지 않는다 — 정정도 반드시 이벤트로 남겨 사람이 판단한다.
