...
잔액·내역의 숫자는 결국 **두 장부**에서 나온다. 하나는 기록(백엔드 DB), 다른 하나는 바깥의 진실인 **Fireblocks 의 값**이다. 매니저 내부 폴링 누락·큐 consume 지연·이쪽 반영 버그는 생기게 마련이라, 회계가 걸리는 숫자는 **주기적으로 둘을 직접 대조**한다. 이 대사는 큐 경로와 무관하게 도는 **독립적 안전장치**다.
