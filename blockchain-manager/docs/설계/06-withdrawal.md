...
승인된 출금 지시가 매니저를 지나 확정되기까지 — 제출·서명·전파·추적, 그리고 막혔을 때.
서명 두 겹(vault 승인·relay 거래)과 서명 직전 검증, boost·cancel 운영, 공통 상태 번역을 다룬다.

업무 승인이 끝난 출금 지시를 Service 백엔드가 매니저에 넘깁니다(제출).
...
submitTransaction(TransactionRequest {
    ...
    travelRule?     // 해외(Notabene) 경로일 때 — 트래블룰 게이트가 만든 암호화 메시지.
                    // 포트는 운반만 하고 내용을 모른다 (트래블룰 문서 10장)
}) → TxRef { id — 벤더 트랜잭션 id }
```
...
sequenceDiagram
    ...
    BE->>FB: submitTransaction(승인된 출금 · gasless)<br/>from = 출금 풀 vault — 출금 전용 · round-robin (5장)
    FB->>FB: createTransaction(…)
    FB-->>BE: 트랜잭션 id (= TxRef)
    ...
    CH-->>FB: 블록 누적 → 확정
    Note over PW,FB: 다시 오프체인 — 폴링은 읽기라 체인에 아무 흔적을 남기지 않는다
    PW->>FB: 폴링 — lastUpdated 커서로 변경된 tx 조회 (4장)
    PW->>DB: 상태 갱신 — TxRef 로 대조, 전파 → 누적 → 확정
```

## 서명은 두 겹 — 안쪽 승인(vault)과 바깥 거래(relay)

다이어그램의 서명 관문(5~8)과 relay 구간(9~10)은 **별개의 서명 두 개**를 만듭니다:

| 서명 | 누가 · 무엇에 | 무엇을 막나 |
|---|---|---|
| **안쪽 — vault 의 승인 서명** (5~8) | 출금 vault(옴니버스·출금 pool)의 MPC 서명 — 벤더 share + **co-signer share**. co-signer 는 Callback Handler 재검증을 통과한 건에만 자기 share 를 보탠다. | 침해된 백엔드의 위조 지시 — 목적지·금액이 정책에 어긋나면 서명 자체가 만들어지지 않는다. |
| **바깥 — relay 의 거래 서명** (9~10) | relay 가 자기 계정으로 바깥 거래를 서명·제출하고 gas 를 낸다. | relay 가 정할 수 있는 건 "낼지 말지"뿐 — 내용 위조는 불가. 위임된 지갑 코드가 안쪽 서명을 온체인에서 검증하기 때문. |
...
제출 다이어그램 6번(승인 질의)에서 확인하는 항목입니다. 하나라도 어긋나면 co-signer 가 자기 share 를 보태지 않아 **서명 자체가 만들어지지 않습니다**.
...
sequenceDiagram
    ...
    Note over PW,DB: 막힌 거래는 변화가 없어 폴링 커서에 다시 안 잡힌다 — 그래서 DB 쪽에서 골라낸다
    PW->>DB: 오래 CONFIRMING 인 건 조회 (벤더 호출 없음)
    PW->>ADM: 막힌 건 보고 — 개입 판단 대상
    ADM->>FB: statusOf(txRef) — 개입 전 단건 확인 (아직 CONFIRMING 인가)
    alt boost — 재촉
        ADM->>FB: boost(txRef) — 벤더 API 로 지시
    ...
    CH-->>FB: 대체 거래 확정
    Note over PW,FB: 이후는 다시 4장 폴링 — 원래 건의 종결과 대체 건의 확정을 실어 온다<br/>auto-boost 미지원이라 감지·지시는 우리 몫이다
```

막힌 출금의 처리 한 사이클. 감지(1~2)는 Service 의 막힘 점검이 DB 에서 골라내고, 개입(3~)은 나가는 돈에 손대는 일이라 Admin 몫이다 — statusOf 로 그 건의 현재 상태를 확인한 뒤 boost(수수료 올려 재전송) 또는 cancel(철회)을 벤더 API 로 지시한다. 실제 대체 거래는 발신자인 relay 가 만들어 전파한다.

## 상태 한 장 — Fireblocks 필드를 공통 어휘로

Fireblocks 는 내부 상태를 여러 단계로 보냅니다(PENDING_SIGNATURE, QUEUED, BROADCASTING, CONFIRMING, COMPLETED, FAILED…). 백엔드가 보는 것은 **어댑터가 번역한 공통 상태** 하나입니다. 아래 넷이 밖으로 나갑니다.

| 공통 상태 (TxStatus) | 뜻 | Fireblocks 원어 (어댑터가 번역) |
|---|---|---|
...
상태 이름과 확정 정책(DCCP)은 벤더 안에 있지만, 그것을 공통 어휘 넷으로 번역하는 어댑터와 막혔을 때의 boost·cancel(Admin) 판단은 이쪽 몫입니다 — 언제 boost 할지, 언제 철회할지 같은 업무 정책은 매니저가 흡수하지 못합니다.

더 깊이: 조립 시 수수료 설정·인상 재전송 — 가이드 4장 · 서명 직전 검증 — 가이드 5.5 절 · 출금 시나리오 — 가이드 11장.

---
출처: [wallet-design-walkthrough/06-withdrawal.html](https://wiki-docs.pages.dev/wallet-design-walkthrough/06-withdrawal.html)
