...
1. ~~스켈레톤 ↔ 가이드 9-1 어긋남~~ — **완료(2026-06-11, commit 16fbde5)**: 아래 누적 항목 전부 코드 반영 (컴파일 미검증 — gradle 부재, 스켈레톤 CLAUDE.md 8절 참조). 원문: 스켈레톤(company/custodial-wallet)엔 "지갑 개설" 묶음 유스케이스가 없음 — `AccountController` 가 createAccount/addressOf 를 별개 엔드포인트·별개 멱등 키로 노출. 제안: `openWallet(ref, asset)` 묶음 메서드 + `POST /wallets` (멱등 키 하나) 추가, `WalletGatewayService` kdoc 의 "API 게이트웨이" 역할 명칭을 가이드 어휘(유스케이스)와 정렬. ★ 추가(2026-06-11, "주소 탄생" 모델 정정 후): SelfBuildAdapter 의 `addressOf` 를 발급 장부(디렉터리) 조회로, 첫 주소도 `issueDepositAddress` 로 발급하는 모델로 동기화 — addressOf 는 절대 새 주소를 만들지 않는다 (가이드 9.1·13.3·15.3 의 새 불변식). ★ 추가(2026-06-11, 코드 정합 리뷰 후): 스켈레톤 chains 모듈에 SPI 선택 capability 2종 설계 반영 — `DepositAddressDerivationCapability.deriveAddress(account, index)` (EVM·UTXO·Solana, 곡선별 결정적 계산만) / `OnLedgerAccountRegistrationCapability.registerAccount(account, pubkey)` (Canton: generate-topology→서명→allocate). 가이드 문서에는 인터페이스를 추가하지 않기로 결정(설명용 의사코드 — 주석으로만 표기, commit 참조). ★ 추가(2026-06-11, 15.2 재구성 후): SelfBuildAdapter 에서 ChainQueryPort 구현을 분리해 별도 `IndexerQueryAdapter` 로 (가이드 15.2 와 동일 — custody 는 Account·Transaction 두 포트만, ChainQuery 는 custody 와 무관한 인덱서 어댑터 한 벌). 어댑터 레지스트리(ChainAdapters)도 생성자 주입으로. ★ 추가(2026-06-11): `TransactionPort` 에 `transactionsOf(account, range)` 신설(가이드 13.3 — 내 거래 이력은 custody 가 줌, 임의 주소만 ChainQuery), 세 어댑터 구현 + `addressOf` 발급 장부 조회화 + `createAccount` 의 Canton `registerAccount` capability 라우팅(가이드 15.2)까지 한 묶음으로 동기화할 것.
2. ~~멱등 레코드 저장 주체 모호~~ — **해소됨(2026-06-11)**: Figure 9-1 삭제 + 다이어그램 호출자를 '지갑 서비스 백엔드' 한 칸으로 통합(commit b5315a2)하면서 GW/SVC 간 라벨 자체가 사라짐.
3. ~~인증 이중 표기~~ — **대부분 해소됨(2026-06-11)**: 호출자 통합으로 그림에서 GW 인증 표기가 사라지고, 역할 분리는 9.1 텍스트(불변식 callout)에만 남음. 잔여도 해소(2026-06-11, commit 16fbde5) — 9.1 불변식 callout 에 호출 시스템/최종 사용자 인증 구분 명시.
4. ~~15.4 송신 흐름 체인별 보강(미결 제안)~~ — **완료(2026-06-11)**: 공통 그림 유지 + 단계×체인 표 + Canton 전용 Figure 15-3b (OFFER→ACCEPT/WITHDRAW).
5. **입금 식별자 재정의의 가이드·스켈레톤 동기화 (Stage 81 후속)**: issueDepositAddress = (주소+memo) 식별자 발급, 전 분면 구현으로 승격 — 가이드 13.3(capability 해제·의미 재정의)·9.2/9.3·14장(FB Canton 행)·15.3(15-2d)·2.4 주석·confluence 04, 스켈레톤 DepositAddressIssuanceCapability 재검토. 워크스루는 반영 완료(2026-06-12).
