...
3. ~~인증 이중 표기~~ — **대부분 해소됨(2026-06-11)**: 호출자 통합으로 그림에서 GW 인증 표기가 사라지고, 역할 분리는 9.1 텍스트(불변식 callout)에만 남음. 잔여: 9.1 의 "게이트웨이 = 인증"이 호출 시스템 인증(비즈의 최종 사용자 인증·KYC 와 다름)임을 한 단어로 명시하면 좋음 — 우선순위 낮음.
